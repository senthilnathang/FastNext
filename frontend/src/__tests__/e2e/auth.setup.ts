...
setup('authenticate regular user', async ({ page }) => {
  try {
    ...
    const usernameSelector = 'input[name="username"], input[name="email"], input[type="email"], input[placeholder*="email" i], input[placeholder*="username" i]';
    ...
    // Submit login form
    const submitButton = 'button[type="submit"], button:has-text("Login"), button:has-text("Sign in")';
