...
test.describe('User Management (Admin)', () => {
  ...
  test('should display users list page', async ({ page }) => {
    ...
    await expect(page.locator('h1, h2, [data-testid="page-title"]')).toContainText(/users/i);

    // Check for users table or list
    const usersTable = page.locator('table, [data-testid="users-table"], .data-table');
  ...
  test('should open create user dialog', async ({ page }) => {
    ...
    if (await createButton.count() > 0) {
      ...
      const dialog = page.locator('[role="dialog"], .modal, [data-testid="create-user-dialog"]');
  ...
  test('should create new user', async ({ page }) => {
    ...
    if (await createButton.count() > 0) {
      ...
      await page.fill('input[name="email"], input[type="email"]', `testuser${Date.now()}@example.com`);
      await page.fill('input[name="username"]', `testuser${Date.now()}`);
      await page.fill('input[name="password"], input[type="password"]', 'TestPassword123');

      // Optional: fill other fields if they exist
      const fullNameField = page.locator('input[name="full_name"], input[name="fullName"]');
  ...
  test('should search users', async ({ page }) => {
    ...
    if (await searchInput.count() > 0) {
      ...
      const tableRows = page.locator('tbody tr, .table-row, [data-testid="user-row"]');
  ...
  test('should edit user', async ({ page }) => {
    ...
    if (await editButton.count() > 0) {
      ...
      const dialog = page.locator('[role="dialog"], .modal, [data-testid="edit-user-dialog"]');
      await expect(dialog).toBeVisible();
