...
test.describe("Workflow Management", () => {
  ...
  test("should display workflows page", async ({ page }) => {
    ...
    ).toContainText(/workflow/i);

    // Check for workflow tabs or sections
    const typeTab = page.locator(
      'button:has-text("Types"), [data-testid="workflow-types-tab"]',
    );
    const templateTab = page.locator(
      'button:has-text("Templates"), [data-testid="workflow-templates-tab"]',
    );
  ...
  test("should switch between workflow tabs", async ({ page }) => {
    ...
    if ((await typeTab.count()) > 0 && (await templateTab.count()) > 0) {
      ...
      const typesContent = page.locator(
        '[data-testid="workflow-types-content"], .workflow-types',
      );
      ...
      const templatesContent = page.locator(
        '[data-testid="workflow-templates-content"], .workflow-templates',
      );
  ...
  test("should create new workflow type", async ({ page }) => {
    ...
    if ((await createTypeButton.count()) > 0) {
      ...
      const dialog = page.locator(
        '[role="dialog"], .modal, [data-testid="create-type-dialog"]',
      );
      ...
      await page.fill(
        ...
        "Test workflow type description",
      );
      ...
      expect(
        !isDialogVisible ||
          (await page.locator("text=success, text=created").count()) > 0,
      ).toBeTruthy();
    }
  });
  ...
  test("should create new workflow template", async ({ page }) => {
    ...
    if ((await createTemplateButton.count()) > 0) {
      ...
      // Check if dialog opens
      const dialog = page.locator(
        '[role="dialog"], .modal, [data-testid="create-template-dialog"]',
      );
      await expect(dialog).toBeVisible();
      ...
      await page.fill(
        'input[name="name"]',
        `Test Workflow Template ${Date.now()}`,
      );
      await page.fill(
        'textarea[name="description"], input[name="description"]',
        "Test workflow template description",
      );

      // Select workflow type if dropdown exists
      const typeSelect = page.locator(
        'select[name="workflow_type_id"], [data-testid="workflow-type-select"]',
      );
  ...
  test("should open workflow builder", async ({ page }) => {
    ...
    if ((await builderButton.count()) > 0) {
      ...
      const builder = page.locator(
        '[data-testid="workflow-builder"], .reactflow-wrapper, .workflow-canvas',
      );
      ...
      if ((await builder.count()) > 0) {
        ...
        const nodesPanel = page.locator(
          '[data-testid="nodes-panel"], .workflow-nodes, .node-palette',
        );
  ...
  test("should display workflow types grid", async ({ page }) => {
    ...
    const typesGrid = page.locator(
      '[data-testid="workflow-types-grid"], .workflow-types-grid, .types-container',
    );
    const typeCards = page.locator(
      '[data-testid="workflow-type-card"], .workflow-type-card, .type-card',
    );
  ...
  test("should display workflow templates grid", async ({ page }) => {
    ...
    const templatesGrid = page.locator(
      '[data-testid="workflow-templates-grid"], .workflow-templates-grid, .templates-container',
    );
    const templateCards = page.locator(
      '[data-testid="workflow-template-card"], .workflow-template-card, .template-card',
    );
  ...
  test("should edit workflow type", async ({ page }) => {
    ...
    if ((await editButton.count()) > 0) {
      ...
      const dialog = page.locator(
        '[role="dialog"], .modal, [data-testid="edit-type-dialog"]',
      );
      await expect(dialog).toBeVisible();
...
test.describe("Workflow Builder", () => {
  test("should load ReactFlow canvas", async ({ page }) => {
    ...
    if ((await builderButton.count()) > 0) {
      ...
      const reactFlowWrapper = page.locator(
        '.react-flow, .reactflow-wrapper, [data-testid="workflow-canvas"]',
      );
      ...
      if ((await reactFlowWrapper.count()) > 0) {
        ...
        const viewport = page.locator(
          ".react-flow__viewport, .reactflow-viewport",
        );
