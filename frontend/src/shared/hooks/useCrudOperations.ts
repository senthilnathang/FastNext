...
import { useCrudNotifications, CrudNotificationOptions } from './useCrudNotifications'
...
export interface CrudOperationsConfig<TData, TCreate, TUpdate, TId = string | number> {
  resourceName: string
  queryKey: string[]
  api: {
...
export function useCrudOperations<TData, TCreate = any, TUpdate = any, TId = string | number>(
  ...
  const { notifyCreateSuccess, notifyUpdateSuccess, notifyDeleteSuccess, notifyError } = useCrudNotifications(notificationOptions)

  // List query
  const useList = (params?: any, options?: UseQueryOptions<{ items: TData[]; total: number }>) => {
    return useQuery({
      queryKey: [...queryKey, params],
      queryFn: () => api.getList ? api.getList(params) : Promise.resolve({ items: [], total: 0 }),
      ...options,
    })
  }
  ...
  // Create mutation
  const useCreate = (options?: Omit<UseMutationOptions<TData, Error, TCreate>, 'onSuccess' | 'onError'>) => {
    return useMutation({
  ...
  // Update mutation
  const useUpdate = (options?: Omit<UseMutationOptions<TData, Error, { id: TId; data: TUpdate }>, 'onSuccess' | 'onError'>) => {
    return useMutation({
      ...
      onSuccess: (data, _variables, _context) => {
        ...
        queryClient.invalidateQueries({ queryKey })
        queryClient.invalidateQueries({ queryKey: [...queryKey, _variables.id] })
      },
      onError: (error, _variables, _context) => {
  ...
  // Delete mutation
  const useDelete = (options?: Omit<UseMutationOptions<void, Error, TId>, 'onSuccess' | 'onError'>) => {
    return useMutation({
      mutationFn: (id: TId) => {
