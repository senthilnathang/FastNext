...
export function useAdvancedSearch<T = any>({
    ...
    searchState.filters.forEach((filter) => {
      if (filter.value !== undefined && filter.value !== "") {
        if (Array.isArray(filter.value)) {
          ...
          params.set(
            `filter_${filter.field}_from`,
            filter.value.from.toISOString(),
          );
          if (filter.value.to) {
            params.set(
              `filter_${filter.field}_to`,
              filter.value.to.toISOString(),
            );
          }
        } else {
  ...
  const hasFilter = React.useCallback(
    (field: string) => {
      return searchState.filters.some(
        (f) => f.field === field && f.value !== undefined && f.value !== "",
      );
    },
    [searchState.filters],
  );
  ...
  const getActiveFilterCount = React.useCallback(() => {
    return searchState.filters.filter(
