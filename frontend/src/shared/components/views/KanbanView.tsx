...
export function KanbanView<T extends { id: string | number }>({
  ...
  }, [cardTitleField, cardDescriptionField, cardFields, canEdit, canDelete, onViewClick, onEditClick, onDeleteClick, customActions])

  const renderColumn = useCallback((column: KanbanColumn, index: number) => {
                  ...
                  >
                    {columnItems.map((item, cardIndex) => (
                      ...
                      >
                        {(provided) => (
                          <div className="group">
                            {renderCard ? renderCard(item, provided) : renderDefaultCard(item, provided)}
                          </div>
                        )}
                      </Draggable>
                    ))}
    ...
    groupedData, allowColumnReorder, showCardCount, showColumnLimits, canCreate, onCreateClick,
    enableQuickAdd, quickAddStates, quickAddValues, canMove, renderCard, renderDefaultCard,
      ...
      <DragDropContext onDragEnd={handleDragEnd}>
        ...
        >
          {(provided) => (
            ...
            >
              {orderedColumns.map((column, index) => renderColumn(column, index))}
              {provided.placeholder}
            </div>
          )}
        </Droppable>
