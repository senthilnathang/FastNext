...
export interface CommonFormViewManagerProps<T = any>
  extends Omit<
    ViewManagerProps<T>,
    | "onCreateClick"
    | "onEditClick"
...
export function CommonFormViewManager<T extends { id?: string | number }>({
  ...
  const handleModeChange = useCallback(
    (newMode: FormViewMode, newItemId?: string | number) => {
      if (onModeChange) {
  ...
  const renderBreadcrumb = () => {
    ...
    if (mode === "create") {
      ...
      breadcrumbs.push({
        label: `Edit ${selectedItem.id}`,
        href: "",
        current: true,
      });
    } else if (mode === "view" && selectedItem) {
      breadcrumbs.push({
        label: `View ${selectedItem.id}`,
        href: "",
        current: true,
      });
    }
  ...
  const renderFormView = () => {
    const formProps: GenericFormViewProps<T> = {
      ...
      submitButtonText:
        mode === "create" ? config.createButtonText : config.editButtonText,
      loading: formLoading,
      error: formError,
    };
              ...
              <div className="flex items-center justify-center h-64">
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    Loading {config.title.slice(0, -1).toLowerCase()}...
                  </p>
                </div>
              </div>
            </CardContent>
...
export function createFormViewConfig<T>(
  config: Partial<FormViewConfig<T>> & {
    resourceName: string;
    baseUrl: string;
    apiEndpoint: string;
    title: string;
