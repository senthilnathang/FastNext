...
export const UserPresence: React.FC<UserPresenceProps> = ({
    ...
    const cleanup = setInterval(() => {
      ...
      setUsers(prev => {
        ...
        for (const [userId, user] of newUsers) {
          ...
          } else if (now - user.lastActive > 60000) { // 1 minute idle
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        } else if (data.type === 'cursor_update' && data.user_id !== currentUserId) {
  ...
  const activeUsers = Array.from(users.values()).filter(user => user.status === 'active');
  const idleUsers = Array.from(users.values()).filter(user => user.status === 'idle');
