...
import { ExportOptions, ExportColumn } from '../types';
...
export function exportToYAML(
  ...
  const yamlData = data.map((row) => {
    ...
    exportColumns.forEach(col => {
      const value = row[col.key];
      exportRow[col.key] = col.type === 'date' && options.dateFormat
        ? formatDateValue(value, options.dateFormat)
        : value;
    });
    return exportRow;
...
export function estimateFileSize(
  ...
  switch (format) {
    ...
    case 'xml':
      sampleContent = sampleData.map(row =>
        exportColumns.map(col => `<${col.key}>${String(row[col.key] || '')}</${col.key}>`).join('')
      ).join('');
      break;
    case 'yaml':
      sampleContent = sampleData.map(row =>
        exportColumns.map(col => `${col.key}: ${String(row[col.key] || '')}`).join('\n')
      ).join('\n');
      break;
    default:
