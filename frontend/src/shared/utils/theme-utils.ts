...
export function getForegroundColor(
  ...
  lightForeground = "oklch(0.961 0 0)",
...
export function getAnalogousColors(
  ...
  const analogous2 = createOklch({
    ...color,
    h: (color.h - spread + 360) % 360,
  });
...
export function hexToOklch(hex: string): string {
  ...
  const l = (r + g + b) / 3;
  const c =
    Math.sqrt((r - l) ** 2 + (g - l) ** 2 + (b - l) ** 2) / Math.sqrt(3);
...
export function getSemanticColor(
  token: keyof typeof semanticColorTokens,
): string {
 ...
 */
export function createThemeFromPrimary(
  primaryColor: string,
): Record<string, string> {
  ...
  return {
    ...
    "secondary-foreground": getForegroundColor(
      adjustLightness(complementary, 0.4),
    ),
    ...
    "muted-foreground": getForegroundColor(
      adjustChroma(adjustLightness(primaryColor, 0.4), -0.1),
    ),
