...
export const optimizedApiClient = {
  async get<T = any>(
    ...
    const cacheConfig = typeof config?.cache === 'object' ? config.cache : defaultCacheConfig
    ...
    if (config?.cache !== false) {
      ...
      if (cacheConfig.staleWhileRevalidate) {
        ...
        if (staleData && globalCache.isStale(cacheKey)) {
          ...
          optimizedApiClient.get(endpoint, options, { ...config, cache: cacheConfig })
            .then(freshData => {
    ...
    const promise = config?.retry !== false
...
export function useOptimizedApiQuery<T>(
  ...
  }, [endpoint, optionsString, configString])
