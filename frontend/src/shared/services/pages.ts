...
export const pagesApi = {
  getProjectPages: async (projectId: number): Promise<Page[]> => {
    const response = await apiClient.get(`${API_CONFIG.ENDPOINTS.PAGES}/project/${projectId}/pages`)
  ...
  updatePage: async (id: number, data: UpdatePageRequest): Promise<Page> => {
    const response = await apiClient.put(`${API_CONFIG.ENDPOINTS.PAGES}/${id}`, data)
    return response.data
  },
