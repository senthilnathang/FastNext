...
import { Edit3, MoreHorizontal, Plus, Shield, UserCheck } from "lucide-react";
...
const UsersPerformancePage: React.FC = () => {
  ...
  const { searchState, updateSearchState } = useAdvancedSearch({
    initialPageSize: 50,
  ...
  const processedUsers = React.useMemo(() => {
    ...
    for (const filter of searchState.filters) {
      ...
      switch (filter.field) {
        case "is_active":
          filtered = filtered.filter(
            (user) => user.is_active === (filter.value === "true"),
          );
          break;
        case "is_verified":
          filtered = filtered.filter(
            (user) => user.is_verified === filter.value,
          );
          break;
        case "is_superuser":
          filtered = filtered.filter(
            (user) => user.is_superuser === filter.value,
          );
          break;
        case "created_at":
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-blue-700 dark:text-blue-300">
                Virtual scrolling enabled - Only rendering visible rows for
                optimal performance
              </p>
            </div>
