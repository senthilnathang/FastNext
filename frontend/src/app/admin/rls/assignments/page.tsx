...
import { z } from "zod";
import {
  type RLSRuleAssignment,
  useCreateRLSAssignment,
  useDeleteRLSAssignment,
  useRLSAssignments,
  useUpdateRLSAssignment,
} from "@/modules/rls/hooks/useRLSAssignments";
...
const formFields: FormField<RLSRuleAssignment>[] = [
  ...
  },
  {
    ...
    description:
      "Specific entity ID (optional - leave empty for all entities of this type)",
  },
  {
    name: "user_id",
    label: "User",
    ...
    options: [], // Will be populated dynamically
    description:
      "Assign to specific user (choose either user or role, not both)",
  },
  {
    name: "role_id",
    label: "Role",
    ...
    options: [], // Will be populated dynamically
    description:
      "Assign to users with this role (choose either user or role, not both)",
  },
  {
    name: "is_active",
    label: "Active",
...
const columns: Column<RLSRuleAssignment>[] = [
  ...
  },
  {
    ...
    render: (value, row) => {
      ...
      if (row.role) {
        return <Badge variant="secondary">Role: {row.role.name}</Badge>;
      }
...
export default function RLSAssignmentsPage() {
  ...
  const createAssignment = async (
    data: RLSRuleAssignment,
  ): Promise<RLSRuleAssignment> => {
  ...
  const updateAssignment = async (
    ...
    return new Promise((resolve, reject) => {
      updateAssignmentMutation.mutate(
        { id: Number(id), data: processedData as any },
        {
          onSuccess: (result) => resolve(result),
          onError: (error) => reject(error),
        },
