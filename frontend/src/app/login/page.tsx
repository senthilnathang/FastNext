  ...
  useEffect(() => {
    ...
    if (redirectReason) {
      const messages: Record<string, string> = {
        ...
        timeout_verified:
          "Your session timed out for security. Please log in again.",
      };

      setTimeoutMessage(
        messages[redirectReason] || "Please log in to continue.",
      );
    }

    if (redirectPath) {
      setTimeoutMessage(
        (prev) => prev || `Please log in to continue to ${redirectPath}.`,
      );
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMessage.includes("expired")) {
        setError(
          "Your password has expired. Please contact an administrator or use password reset if available.",
        );
      } else if (errorMessage.includes("change is required")) {
        setError(
          "Password change is required. Please update your password in settings.",
        );
                ...
                <div className="flex">
                  ...
                  <div className="ml-3 flex-1">
                    ...
                    {(error.includes("expired") ||
                      error.includes("change is required")) && (
                      <div className="mt-3">
                        <Link href="/settings">
                          <Button
                            variant="outline"
