...
export const permissionsRouter = router({
      ...
      try {
        ...
        if (input.resource) {
          filteredPermissions = filteredPermissions.filter(
            (permission: Permission) => permission.resource === input.resource,
          );
        }
  ...
  getById: protectedProcedure.input(z.number()).query(async ({ input: id }) => {
    try {
      ...
      const result = await permissionOperations.getAll();
      const permission = result.permissions.find(
        (permission: Permission) => permission.id === id,
      );
      if (!permission) {
      ...
      throw new Error(
        `Failed to fetch permission: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }),
  ...
  create: protectedProcedure
    ...
    .mutation(async () => {
      try {
        ...
        throw new Error(
          "Create permission mutation not implemented in GraphQL",
        );
      } catch (error) {
        throw new Error(
          `Failed to create permission: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }),
  ...
  update: protectedProcedure
    ...
    .mutation(async () => {
      try {
        ...
        throw new Error(
          "Update permission mutation not implemented in GraphQL",
        );
      } catch (error) {
        throw new Error(
  ...
  delete: protectedProcedure.input(z.number()).mutation(async () => {
    try {
  ...
  getByResource: protectedProcedure
    ...
    .query(async ({ input: resource }) => {
      try {
        const result = await permissionOperations.getAll();
        const resourcePermissions = result.permissions.filter(
        ...
        throw new Error(
          `Failed to fetch permissions by resource: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }),
  ...
  getResources: protectedProcedure.query(async () => {
    try {
      ...
      throw new Error(
        `Failed to fetch permission resources: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }),
});
