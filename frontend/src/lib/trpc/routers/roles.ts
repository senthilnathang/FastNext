...
export const rolesRouter = router({
  ...
  create: protectedProcedure.input(createRoleSchema).mutation(async () => {
    try {
  ...
  search: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        const result = await roleOperations.getAll();
        const filteredRoles = result.roles.filter(
          (role: Role) =>
        ...
        throw new Error(
          `Failed to search roles: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }),
});
