...
export const SECURE_COOKIE_DEFAULTS: Required<
  Omit<CookieSecurityOptions, "expires" | "domain">
> = {
  httpOnly: true,
  secure:
    (process.env.NODE_ENV === "production" &&
      process.env.BYPASS_HTTPS_CHECK !== "true") ||
...
export class SecureCookieManager {
  ...
  private static serializeCookieOptions(
  ...
  static setSecureCookie(
    ...
    SecureCookieManager.validateCookieName(name);
    SecureCookieManager.validateCookieValue(value);

    const cookieOptions = { ...SECURE_COOKIE_DEFAULTS, ...options };
    const serializedOptions =
      SecureCookieManager.serializeCookieOptions(cookieOptions);
  ...
  static setAuthCookie(
    ...
    const config =
      type === "session" ? SESSION_COOKIE_CONFIG : REFRESH_TOKEN_CONFIG;
    const cookieName = type === "session" ? "auth-token" : "refresh-token";

    SecureCookieManager.setSecureCookie(response, cookieName, token, config);
  }
  ...
  static clearAuthCookies(response: NextResponse): void {
    SecureCookieManager.clearCookie(response, "auth-token");
    SecureCookieManager.clearCookie(response, "refresh-token");
    SecureCookieManager.clearCookie(response, "csrf-token");
  }
  ...
  static getAuthToken(request: NextRequest): string | null {
    return SecureCookieManager.getCookie(request, "auth-token");
  }

  static getRefreshToken(request: NextRequest): string | null {
    return SecureCookieManager.getCookie(request, "refresh-token");
  }

  static getCSRFToken(request: NextRequest): string | null {
    return SecureCookieManager.getCookie(request, "csrf-token");
  }
