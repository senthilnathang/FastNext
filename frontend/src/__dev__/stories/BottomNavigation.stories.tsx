...
const meta: Meta<typeof BottomNavigation> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          "A modern bottom navigation component for mobile devices with badge support and overflow handling.",
      },
    },
  },
  argTypes: {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-20">
      <div className="p-6">
        ...
        <div className="text-gray-600 dark:text-gray-400 space-y-2">
          ...
          <p>
            Try tapping different navigation items to see the active state
            change.
          </p>
          {items.length > 5 && (
            <p className="text-blue-600 dark:text-blue-400">
              This example has overflow items - tap &quot;More&quot; to see
              additional options.
            </p>
          )}
        </div>
          ...
          {Array.from({ length: 20 }, (_, i) => (
            ...
            >
              ...
              <p className="text-gray-600 dark:text-gray-400">
                ...
                When hideOnScroll is enabled, the bottom navigation will hide
                when scrolling down.
              </p>
            </div>
          ))}
...
export const Interactive: Story = {
  render: () => {
    ...
    const addBadge = (itemId: string) => {
      setItems((prev) =>
        prev.map((item) =>
          item.id === itemId ? { ...item, badge: (item.badge || 0) + 1 } : item,
        ),
      );
    };
    ...
    const clearBadge = (itemId: string) => {
      setItems((prev) =>
        prev.map((item) =>
          item.id === itemId ? { ...item, badge: undefined } : item,
        ),
      );
    };
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="text-gray-600 dark:text-gray-400">
              <p>
                Active: <strong>{activeItem}</strong>
              </p>
              <p>
                Visible items: <strong>{maxVisible}</strong>
              </p>
              <p>
                Show labels: <strong>{showLabels ? "Yes" : "No"}</strong>
              </p>
            </div>
          </div>
        </div>
