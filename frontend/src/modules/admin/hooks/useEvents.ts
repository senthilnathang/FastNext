...
const eventsApi = {
  ...
  getEvents: async (params: {
    ...
    const response = await fetch(
      `${API_BASE}/events?${searchParams.toString()}`,
      {
        credentials: "include",
        headers: {
  ...
  getEventStatistics: async (hours: number = 24): Promise<EventStatistics> => {
    const response = await fetch(
      `${API_BASE}/events/statistics?hours=${hours}`,
      {
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch event statistics: ${response.statusText}`,
      );
    }
  ...
  exportEvents: async (params: EventExportRequest): Promise<Blob> => {
    ...
    const response = await fetch(
      `${API_BASE}/events/export?${searchParams.toString()}`,
      {
        credentials: "include",
        headers: {
  ...
  // Get logs from file
  getLogsFromFile: async (params: { date?: string; lines?: number }) => {
    const searchParams = new URLSearchParams();
    ...
    const response = await fetch(
      `${API_BASE}/logs?${searchParams.toString()}`,
      {
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
  ...
  getEventCategories: async () => {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch event categories: ${response.statusText}`,
      );
    }
...
export const useLogsFromFile = (
  params: { date?: string; lines?: number } = {},
) => {
  return useQuery({
...
export const useExportEvents = () => {
  return useMutation({
    ...
    onSuccess: (blob, variables) => {
      ...
      toast.success(
        `Events exported successfully as ${variables.format.toUpperCase()}`,
      );
    },
    onError: (error) => {
