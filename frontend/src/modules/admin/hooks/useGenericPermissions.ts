...
export const useGenericPermissions = (
  ...
  const fetchPermissions = useCallback(async () => {
    ...
    try {
      ...
      const permissionObjects: Permission[] = userPermissions.map(
        (permName: string, index: number) => ({
          id: index + 1,
          name: permName,
  ...
  const hasPermission = (check: PermissionCheck): boolean => {
    ...
    const hasSystemPermission = permissions.some(
      (permission) =>
        permission.action === check.action && permission.category === "system",
    );
