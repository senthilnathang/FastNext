...
import type { WorkflowEdge, WorkflowNode } from "../types/reactflow";
  ...
  ({
    ...
    React.useEffect(() => {
      if (templateData) {
        ...
        if (
          templateData.nodes &&
          Array.isArray(templateData.nodes) &&
    ...
    const onConnect = useCallback(
      (params: Connection) => {
        ...
        if (params.source === params.target && sourceNode.type !== "loop")
          return;
          ...
          <div className="flex items-center space-x-2">
            ...
            {templateId && (
              <span className="text-sm text-gray-500">
                ...
                {templateData && (
                  <span className="ml-2 text-xs">({templateData.name})</span>
                )}
              </span>
            )}
                    ...
                    <CardContent className="pt-2">
                      <div className="space-y-1">
                        {validationIssues.map((issue, index) => (
                          <div
                            key={index}
                            className="text-xs text-orange-600 flex items-start space-x-1"
                          >
...
export default function AdvancedWorkflowBuilder(
  props: AdvancedWorkflowBuilderProps,
) {
  return (
    <ReactFlowProvider>
