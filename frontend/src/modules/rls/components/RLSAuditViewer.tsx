...
export default function RLSAuditViewer() {
  ...
  const fetchAuditLogs = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${getApiUrl("/api/v1/rls/audit-logs")}?${params}`,
        {
          headers: {
  ...
  const exportLogs = async () => {
    try {
      ...
      alert(
        "Export functionality would download the filtered audit logs as CSV/JSON",
      );
    } catch {
              ...
              <select
                ...
                value={
                  filters.access_granted === undefined
                    ? ""
                    : filters.access_granted.toString()
                }
                onChange={(e) =>
                  handleFilterChange(
                    "access_granted",
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label>Action</Label>
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full mt-1 ${getActionColor(selectedLog.action)}`}
                    >
                      {selectedLog.action}
                    </span>
                  </div>
