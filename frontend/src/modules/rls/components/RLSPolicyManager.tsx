...
export default function RLSPolicyManager() {
  ...
  const fetchPolicies = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${getApiUrl("/api/v1/rls/policies")}?${params}`,
        {
          headers: {
  ...
  const handleDelete = async (policyId: number) => {
    if (
      !confirm(
        "Are you sure you want to delete this RLS policy? This action cannot be undone.",
      )
    ) {
      return;
    }
  ...
  const handleToggleActive = async (policy: RLSPolicy) => {
    try {
      ...
      const response = await fetch(
        `${getApiUrl("/api/v1/rls/policies")}/${policy.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const getPolicyTypeIcon = (policyType: PolicyType) => {
    switch (policyType) {
      case PolicyType.OWNER_ONLY:
        return <Lock className="h-4 w-4 text-red-600" />;
      case PolicyType.ORGANIZATION_MEMBER:
        return <Users className="h-4 w-4 text-blue-600" />;
      case PolicyType.PROJECT_MEMBER:
        return <Users className="h-4 w-4 text-green-600" />;
      case PolicyType.ROLE_BASED:
        return <Shield className="h-4 w-4 text-purple-600" />;
      case PolicyType.CONDITIONAL:
        return <Settings className="h-4 w-4 text-orange-600" />;
      case PolicyType.PUBLIC:
        return <Eye className="h-4 w-4 text-gray-600" />;
      case PolicyType.TENANT_ISOLATED:
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFilterEntityType(e.target.value as EntityType | "")
              }
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center space-x-3 mb-2">
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-medium ${getActionColor(policy.action)}`}
                  >
                    {policy.action}
                  </span>
                  <span
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      {...register("priority", {
                        ...
                        max: {
                          value: 1000,
                          message: "Priority must be at most 1000",
                        },
                      })}
                      placeholder="100"
                    />
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      {...register("custom_condition", {
                        required:
                          watchPolicyType === PolicyType.CONDITIONAL
                            ? "Custom condition is required for conditional policies"
                            : false,
                      })}
                    ...
                    {errors.custom_condition && (
                      <p className="text-sm text-red-600 mt-1">
                        {errors.custom_condition.message}
                      </p>
                    )}
                    <p className="text-xs text-gray-500 mt-1">
