...
export interface UpdateRLSAssignmentRequest
  extends Partial<CreateRLSAssignmentRequest> {
  is_active?: boolean;
}
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<RLSRuleAssignment> => {
      const response = await apiClient.get(
        `/api/v1/rls/rule-assignments/${id}`,
      );
      return response.data;
    },
    enabled: !!id,
...
export function useCreateRLSAssignment() {
  ...
  return useMutation({
    mutationFn: async (
      data: CreateRLSAssignmentRequest,
    ): Promise<RLSRuleAssignment> => {
      const response = await apiClient.post(
        "/api/v1/rls/rule-assignments",
        data,
      );
      return response.data;
    },
...
export function useUpdateRLSAssignment() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      id: number;
      data: UpdateRLSAssignmentRequest;
    }): Promise<RLSRuleAssignment> => {
