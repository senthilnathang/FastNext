...
import { usePageComponents, useCreateComponentInstance, useUpdateComponentInstance } from '@/modules/builder/hooks/useComponents'
...
export default function UIBuilder({ projectId, pageId }: UIBuilderProps) {
  ...
  const selectedInstance = selectedInstanceId 
    ? instances.find((instance: any) => instance.id === selectedInstanceId) || null
    : null
    ...
    if (activeData?.type === 'component-instance' && overData?.type === 'component-instance') {
      ...
      if (activeInstance.parent_id === overInstance.parent_id) {
        const siblings = instances.filter(
          (instance: any) => instance.parent_id === activeInstance.parent_id
        ).sort((a: any, b: any) => a.order_index - b.order_index)

        const oldIndex = siblings.findIndex((instance: any) => instance.id === activeInstance.id)
        const newIndex = siblings.findIndex((instance: any) => instance.id === overInstance.id)
        ...
        if (oldIndex !== newIndex) {
          ...
          for (let i = 0; i < newOrder.length; i++) {
            const instance = newOrder[i] as any
            if (instance.order_index !== i) {
              try {
