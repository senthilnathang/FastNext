...
export default function SecuritySettings() {
  ...
  const [settingsBackup, setSettingsBackup] =
    useState<SecuritySettingsData | null>(null);
  ...
  const fetchSecuritySettings = async () => {
    try {
      ...
      const response = await fetch(
        getApiUrl(API_CONFIG.ENDPOINTS.SECURITY.SETTINGS),
        {
          headers: {
  ...
  const fetchSecurityOverview = async () => {
    try {
      const token = localStorage.getItem("access_token");
      const response = await fetch(
        getApiUrl(API_CONFIG.ENDPOINTS.SECURITY.OVERVIEW),
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const onSubmit = async (data: SecuritySettingsData) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.detail || "Failed to update security settings",
        );
      }
  ...
  const handleDisable2FA = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("access_token");
      const response = await fetch(
        getApiUrl(API_CONFIG.ENDPOINTS.SECURITY.TWO_FA_DISABLE),
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleResetToDefaults = async () => {
    if (
      !confirm(
        "Are you sure you want to reset all security settings to defaults? This action cannot be undone.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.detail || "Failed to reset security settings",
        );
      }
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (importData.security_settings) {
          if (
            confirm(
              "Are you sure you want to import these security settings? This will overwrite your current settings.",
            )
          ) {
            Object.entries(importData.security_settings).forEach(
              ([key, value]) => {
                setValue(
    ...
    <div className="space-y-6">
      ...
      {overview && (
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div
              className={`p-4 rounded-lg border-2 ${getSecurityScoreBg(overview.security_score)}`}
            >
              ...
              <div
                className={`text-2xl font-bold ${getSecurityScoreColor(overview.security_score)}`}
              >
            ...
            <div className="p-4 bg-gray-50 rounded-lg border">
              ...
              <div
                className={`text-lg font-semibold ${overview.two_factor_enabled ? "text-green-600" : "text-red-600"}`}
              >
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="max_session_duration_hours">
        ...
        <Card className="p-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center space-x-2">
              <Checkbox id="email_on_login" {...register("email_on_login")} />
            ...
            <div className="mt-3 space-y-1">
              {settingsBackup && (
                <p className="text-xs text-green-600">
                  Settings backup created. You can restore your previous
                  configuration.
                </p>
              )}
              <p className="text-xs text-gray-500">
                Use these tools to manage your security configuration.
                Export/Import allows you to transfer settings between devices.
              </p>
            </div>
          </Card>
        </div>
