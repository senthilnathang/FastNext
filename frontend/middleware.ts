...
const ROUTE_CONFIG = {
  ...
  protected: ["/dashboard", "/admin", "/settings", "/projects", "/workflows"],
  ...
  development: ["/storybook", "/__dev__", "/test"],
...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    const cookieValidation =
      SecureCookieManager.validateCookieSecurity(request);
    if (!cookieValidation.isValid) {
      ...
      return createSecurityResponse(
        "Invalid cookie security",
        "COOKIE_SECURITY_VIOLATION",
        400,
        requestId,
      );
    }
    ...
    if (xssDetection.detected) {
      console.warn(
        `XSS attempt detected: ${xssDetection.patterns.join(", ")}`,
        {
          requestId,
          clientIP,
          userAgent,
    ...
    const authResult = await handleRouteAuthorization(
      request,
      pathname,
      requestId,
    );
...
function isDevelopmentRoute(pathname: string): boolean {
  return ROUTE_CONFIG.development.some(
...
function requiresAuthentication(pathname: string): boolean {
  return [
    ...
    ...ROUTE_CONFIG.admin,
    ...ROUTE_CONFIG.apiProtected,
  ].some(
    (route) => pathname.startsWith(route) || matchesPattern(pathname, route),
  );
}

function requiresAdminAccess(pathname: string): boolean {
  return ROUTE_CONFIG.admin.some(
    (route) => pathname.startsWith(route) || matchesPattern(pathname, route),
  );
}
...
async function checkAuthentication(request: NextRequest): Promise<{
  ...
  const token =
    SecureCookieManager.getAuthToken(request) ||
...
function logRequest(
  ...
  if (
    ...
    (status >= 400 || processingTime > 2000)
  ) {
