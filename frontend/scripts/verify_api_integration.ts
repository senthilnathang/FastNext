...
class APIIntegrationVerifier {
  ...
  private checkFile(filePath: string): CheckResult | null {
    ...
    if (
      ...
      !filePath.includes("api/client") &&
      !filePath.includes("api/config")
    ) {
  ...
  private checkForOldAPIPatterns(filePath: string): CheckResult | null {
    ...
    const problematicPatterns = [
      {
        pattern: /apiClient\.(get|post|put|delete)\(['"`]\/(?!api\/v1\/)/g,
        description: "Direct endpoint without /api/v1 prefix",
      },
      {
        pattern: /['"`]\/api\/(?!v1\/)/g,
        description: "API call without v1 versioning",
      },
      {
        pattern:
          /fetch\(['"`][^'"`]*\/(?:users|roles|permissions|projects)(?!\/api\/v1)/g,
        description: "Direct fetch without proper API structure",
      },
    ];
