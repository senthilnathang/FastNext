...
from scaffolding.backend_generator import (
    ...
    ModelDefinition as BackendModelDefinition,
    FieldDefinition as BackendFieldDefinition,
    FieldType,
    ModelMixin,
    ValidationRule,
    RelationshipConfig
...
Examples:
  ...
  python scaffold-cli.py example-config --name Product
        ...
        gen_parser.add_argument('--name', type=str, help='Model name (e.g., Product, BlogPost)')
        gen_parser.add_argument('--type', choices=['frontend', 'backend', 'both'], 
                               default='both', help='What to generate')
        gen_parser.add_argument('--config', type=str, help='JSON config file path')
        gen_parser.add_argument('--output-frontend', type=str, 
                               default=str(self.frontend_path), help='Frontend output path')
        gen_parser.add_argument('--output-backend', type=str,
                               default=str(self.backend_path), help='Backend output path')
        ...
        config_parser = subparsers.add_parser('example-config', help='Generate example config')
    ...
    def generate_scaffolding(self, args):
        ...
        if args.type in ['frontend', 'both']:
            try:
                ...
                frontend_generator = FrontendScaffoldGenerator(frontend_model, args.output_frontend)
        ...
        if args.type in ['backend', 'both']:
            try:
                ...
                backend_generator = BackendScaffoldGenerator(backend_model, args.output_backend)
    ...
    def generate_example_config(self, args):
        ...
        print(f"🔧 Edit the file and run: python scaffold-cli.py generate --config {output_file}")
    ...
    def interactive_builder(self):
        ...
        while True:
            ...
            print("  Available types: string, number, boolean, date, email, url, text, select, multiselect")
