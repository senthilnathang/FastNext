...
class WorkflowEngine:
    ...
    async def execute_action(
        ...
        if instance.status not in [InstanceStatus.RUNNING, InstanceStatus.PENDING]:
            raise WorkflowExecutionError(
                f"Instance {instance_id} is not in a runnable state"
            )
        ...
        if not valid_transitions:
            raise WorkflowExecutionError(
                f"Action '{action}' not allowed from current state"
            )
    ...
    def _get_valid_transitions(
        self, template: WorkflowTemplate, current_state_id: int, action: str
    ) -> List[Dict[str, Any]]:
        ...
        for edge in template.edges:
            ...
            if (
                source_node
                and source_node.get("data", {}).get("stateId") == current_state_id
                and edge.get("data", {}).get("action") == action
            ):
    ...
    def _find_node_by_id(
        self, template: WorkflowTemplate, node_id: str
    ...
    def _check_permissions(
        self, template: WorkflowTemplate, action: str, user_id: int
    ) -> bool:
    ...
    async def _execute_state_actions(
        self, instance: WorkflowInstance, target_node: Dict[str, Any]
    ):
    ...
    async def _execute_service_task(
        self, instance: WorkflowInstance, service_name: str, config: Dict[str, Any]
    ):
        ...
        logger.info(
            f"Executing service task '{service_name}' for workflow instance {instance.id}"
        )
        ...
        if instance.deadline and datetime.utcnow() > instance.deadline:
            ...
            for edge in template.edges:
                if edge.get("data", {}).get("action") == "timeout":
                    ...
                    if (
                        source_node
                        and source_node.get("data", {}).get("stateId")
                        ...
                        transition = {
                            "edge": edge,
                            "source": source_node,
                            "target": self._find_node_by_id(
                                template, edge.get("target")
                            ),
                            "target_state_id": self._find_node_by_id(
                                template, edge.get("target")
                            )
                            .get("data", {})
                            .get("stateId"),
    ...
    def _is_workflow_complete(self, instance: WorkflowInstance) -> bool:
        ...
        current_state = (
            self.db.query(WorkflowState)
            .filter(WorkflowState.id == instance.current_state_id)
            .first()
        )
