...
class PasswordService:
    ...
    def validate_password_policy(
        ...
        if not security_settings:
            security_settings = (
                self.db.query(SecuritySetting)
                .filter(SecuritySetting.user_id == user.id)
                .first()
            )
        ...
        if len(password) < security_settings.min_password_length:
            errors.append(
                f"Password must be at least {security_settings.min_password_length} characters long"
            )
        ...
        if security_settings.require_special_chars and not re.search(
            r'[!@#$%^&*(),.?":{}|<>]', password
        ):
        ...
        if security_settings.password_history_count > 0:
            if self._is_password_in_history(
                password, user.id, security_settings.password_history_count
            ):
                errors.append(
                    f"Password cannot be one of your last {security_settings.password_history_count} passwords"
                )
    ...
    def _is_password_in_history(
        self, password: str, user_id: int, history_count: int
    ) -> bool:
        """
    ...
    def update_password_history(self, user_id: int, new_password_hash: str) -> None:
        ...
        security_settings = (
            self.db.query(SecuritySetting)
            .filter(SecuritySetting.user_id == user_id)
            .first()
        )

        history_count = (
            security_settings.password_history_count if security_settings else 5
        )

        # Add new password to history
        history_entry = PasswordHistory(
            user_id=user_id, hashed_password=new_password_hash
        )
    ...
    def check_password_expiry(self, user: User) -> Tuple[bool, Optional[int]]:
        ...
        expiry_date = user.password_changed_at + timedelta(
            days=security_settings.password_expiry_days
        )
        now = datetime.utcnow()
