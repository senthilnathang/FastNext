...
class PermissionService:
    ...
    def get_user_permissions(db: Session, user_id: int) -> List[Permission]:
        ...
        project_memberships = (
            ...
            .filter(ProjectMember.user_id == user_id, ProjectMember.is_active == True)
            .all()
        )
    ...
    def check_permission(
        ...
        for permission in permissions:
            if permission.action == action and permission.category == category:
                return True

            # Check for manage permission (implies all actions)
            if permission.action == "manage" and permission.category == category:
                return True
    ...
    def check_project_permission(
        db: Session, user_id: int, project_id: int, action: str
    ) -> bool:
        ...
        for rp in role_permissions:
            ...
            if permission.action == action and permission.category == "project":
                return True

            if permission.action == "manage" and permission.category == "project":
                return True
    ...
    @staticmethod
    def assign_role(
        db: Session, user_id: int, role_id: int, assigned_by: int
    ) -> UserRole:
    ...
    def add_project_member(
        db: Session, project_id: int, user_id: int, role_id: int, invited_by: int
    ) -> ProjectMember:
    ...
    def check_resource_permission(
        ...
        for permission in permissions:
            # Check exact match
            if permission.action == action and permission.category == resource_type:
                return True

            # Check for manage permission (implies all actions)
            if permission.action == "manage" and permission.category == resource_type:
                return True

            # Check for system-wide permissions
            if permission.action == action and permission.category == "system":
                return True
    ...
    def get_allowed_actions(
        db: Session, user_id: int, resource_type: str, project_id: Optional[int] = None
    ) -> List[str]:
