...
class AuthorService:
    ...
    def update_author(self, author_id: int, author_data: AuthorUpdate, user: User) -> Author:
    ...
    def list_authors(self, skip: int = 0, limit: int = 100, search: Optional[str] = None, user: User = None) -> List[Author]:
        ...
        if search:
            ...
            query = query.filter(db.or_(Author.name.ilike(f'%{search}%'), Author.description.ilike(f'%{search}%')))
