...
from pydantic import BaseModel, ValidationError, validator, EmailStr, HttpUrl, Field
...
class ValidationService:
    ...
    @staticmethod
    def validate_tags(tags: List[str], max_tags: int = 10, max_tag_length: int = 50) -> Dict[str, Any]:
        """Validate tags array"""
        errors = []
        ...
        for tag in tags:
            if len(tag) > max_tag_length:
                errors.append(f"Tag '{tag}' is too long (max {max_tag_length} characters)")
    ...
    @classmethod
    def validate_pydantic_model(cls, model_class: Type[BaseModel], data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    @classmethod
    def create_field_validator(cls, validation_func, error_message: str = "Validation failed"):
...
def UuidField(**kwargs) -> str:
    ...
    return Field(regex=r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$', **kwargs)
