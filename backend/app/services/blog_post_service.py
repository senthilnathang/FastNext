...
class BlogPostService:
    ...
    def update_blog_post(self, blog_post_id: int, blog_post_data: BlogPostUpdate, user: User) -> BlogPost:
    ...
    def list_blog_posts(self, skip: int = 0, limit: int = 100, search: Optional[str] = None, user: User = None) -> List[BlogPost]:
        ...
        if search:
            ...
            query = query.filter(db.or_(BlogPost.title.ilike(f'%{search}%'), BlogPost.excerpt.ilike(f'%{search}%'), BlogPost.content.ilike(f'%{search}%')))
