...
class ProductService:
    ...
    def update_product(self, product_id: int, product_data: ProductUpdate, user: User) -> Product:
    ...
    def list_products(self, skip: int = 0, limit: int = 100, search: Optional[str] = None, user: User = None) -> List[Product]:
        ...
        if search:
            ...
            query = query.filter(db.or_(Product.name.ilike(f'%{search}%'), Product.description.ilike(f'%{search}%')))
