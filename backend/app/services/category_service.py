...
class CategoryService:
    ...
    def update_category(self, category_id: int, category_data: CategoryUpdate, user: User) -> Category:
    ...
    def list_categorys(self, skip: int = 0, limit: int = 100, search: Optional[str] = None, user: User = None) -> List[Category]:
        ...
        if search:
            ...
            query = query.filter(db.or_(Category.name.ilike(f'%{search}%'), Category.description.ilike(f'%{search}%')))
