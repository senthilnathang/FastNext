...
from app.schemas.component import (
    ...
    ComponentCreate,
    ComponentUpdate,
    ComponentInstance as ComponentInstanceSchema,
...
def read_page_component_instances(
    *,
    ...
    instances = db.query(ComponentInstance).filter(
        ComponentInstance.page_id == page_id
    ).order_by(ComponentInstance.order_index).all()
    return instances
...
def create_component_instance(
    *,
    ...
    component = db.query(Component).filter(Component.id == instance_in.component_id).first()
    if not component:
        raise HTTPException(status_code=404, detail="Component not found")
...
def update_component_instance(
    *,
    ...
    instance = db.query(ComponentInstance).join(Page).join(Project).filter(
        ComponentInstance.id == instance_id,
        Project.user_id == current_user.id
    ).first()
    if not instance:
