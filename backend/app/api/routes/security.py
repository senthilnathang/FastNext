...
from app.schemas.security_setting import (
    SecuritySettingResponse, SecuritySettingUpdate, SecuritySettingCreate,
    TwoFactorSetup, TwoFactorVerify, TwoFactorDisable, SecuritySettingsOverview
)
...
def get_security_settings(
    ...
    security_settings = db.query(SecuritySetting).filter(
        SecuritySetting.user_id == current_user.id
    ).first()
    ...
    *,
    ...
    try:
        ...
        log_security_event(
            ...
            extra_data={"updated_fields": list(settings_in.dict(exclude_unset=True).keys())}
        )
...
def disable_two_factor_auth(
    *,
    ...
    if not security.verify_password(disable_data.password, current_user.hashed_password):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if disable_data.token:
        ...
        if not (totp.verify(disable_data.token, valid_window=2) or disable_data.token in backup_codes):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid verification code"
            )
...
def get_security_overview(
    ...
    if current_user.password_changed_at:
        days_since_password_change = (datetime.utcnow() - current_user.password_changed_at.replace(tzinfo=None)).days
        if days_since_password_change > 90:
            recommendations.append("Consider changing your password (last changed over 90 days ago)")
    else:
        recommendations.append("Set a password change date for better security tracking")
    ...
    return SecuritySettingsOverview(
        user_id=current_user.id,
        two_factor_enabled=security_settings.two_factor_enabled if security_settings else False,
