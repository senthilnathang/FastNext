...
class VersionedAPIManager:
    ...
    def _add_version_headers(
        self, response: Response, detected_version: str, request: Request
    ):
        ...
        if (
            hasattr(request.state, "deprecation_warning")
            and request.state.deprecation_warning
        ):
        ...
        for ver in self.supported_versions:
            if self.strategy == VersioningStrategy.URL_PATH:
                ...
                response.headers[f"Link-Version-{ver}"] = (
                    f'<{version_url}>; rel="version"'
                )

    def _create_version_error_response(
        self, message: str, error_code: str, requested_version: str
    ) -> JSONResponse:
    ...
    def deprecate_version(
        self, version_str: str, sunset_date: Optional[date] = None, reason: str = ""
    ):
...
def deprecated_in_version(
    version_str: str, reason: str = "", migration_guide: str = ""
):
...
class VersionCompatibilityChecker:
    ...
    def register_breaking_change(
        self, from_version: str, to_version: str, changes: List[str]
    ):
    ...
    def register_field_mapping(
        self, from_version: str, to_version: str, field_map: Dict[str, str]
    ):
...
class VersionedContentNegotiator:
    ...
    def register_serializer(
        self, version: str, content_type: str, serializer: Callable
    ):
