...
from app.schemas.workflow import WorkflowTemplate as WorkflowTemplateSchema
...
def read_workflow_templates(
    ...
    try:
        ...
        return ListResponse.paginate(
            items=templates, total=total, skip=skip, limit=limit
        )
...
def create_workflow_template(
    *,
    ...
    if not PermissionService.check_permission(
        db, current_user.id, "create", "workflow_template"
    ):
        raise HTTPException(
    ...
    workflow_type = (
        ...
        .filter(WorkflowType.id == template_in.workflow_type_id)
        .first()
    )
    ...
    if template_in.default_state_id:
        default_state = (
            ...
            .filter(WorkflowState.id == template_in.default_state_id)
            .first()
        )
        if not default_state:
            raise HTTPException(status_code=404, detail="Default state not found")

    template = WorkflowTemplate(**template_in.dict(), created_by=current_user.id)
    db.add(template)
    db.commit()
    db.refresh(template)
    return template
...
def read_workflow_template(
    *,
    ...
    template = (
        db.query(WorkflowTemplate).filter(WorkflowTemplate.id == template_id).first()
    )
    if not template:
...
def update_workflow_template(
    *,
    ...
    if not PermissionService.check_permission(
        db, current_user.id, "update", "workflow_template"
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
def delete_workflow_template(
    *,
    ...
    if not PermissionService.check_permission(
        db, current_user.id, "delete", "workflow_template"
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
