...
from app.schemas.workflow import WorkflowInstance as WorkflowInstanceSchema
from app.schemas.workflow import WorkflowInstanceCreate, WorkflowInstanceUpdate
from app.services.permission_service import PermissionService
from app.services.workflow_engine import WorkflowExecutionError, get_workflow_engine
...
def read_workflow_instances(
    ...
    try:
        ...
        if not PermissionService.check_permission(
            db, current_user.id, "read", "all_workflow_instances"
        ):
            query = query.filter(
        ...
        return ListResponse.paginate(
            items=instances, total=total, skip=skip, limit=limit
        )
...
async def create_workflow_instance(
    *,
    ...
    if not PermissionService.check_permission(
        db, current_user.id, "create", "workflow_instance"
    ):
        raise HTTPException(
    ...
    template = (
        ...
        .filter(WorkflowTemplate.id == instance_in.template_id)
        .first()
    )
    if not template or not template.is_active:
        raise HTTPException(
            status_code=404, detail="Active workflow template not found"
        )
...
def read_workflow_instance(
    *,
    ...
    instance = (
        db.query(WorkflowInstance).filter(WorkflowInstance.id == instance_id).first()
    )
    if not instance:
    ...
    if not PermissionService.check_permission(
        ...
        if (
            ...
            and instance.assigned_to != current_user.id
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
async def execute_workflow_action(
    *,
    ...
    if not PermissionService.check_permission(
        db, current_user.id, "update", "workflow_instance"
    ):
        if (
            instance.created_by != current_user.id
            and instance.assigned_to != current_user.id
...
async def process_pending_workflows(
    ...
    if not PermissionService.check_permission(
        db, current_user.id, "admin", "workflow_processing"
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    for edge in template.edges:
        ...
        for node in template.nodes:
            if (
                ...
                and node.get("data", {}).get("stateId") == instance.current_state_id
            ):
                source_node = node
                break
