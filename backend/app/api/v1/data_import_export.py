...
from app.schemas.data_import_export import (
    ...
    ExportJobResponse,
    ExportJobUpdate,
    ExportPermissionCreate,
    ExportPermissionResponse,
...
async def upload_and_create_import_job(
    ...
    logger.info(
        f"👤 User: {current_user.id} ({current_user.email if hasattr(current_user, 'email') else 'N/A'})"
    )
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"✅ Import options parsed: format={options.format}, has_headers={options.has_headers}"
            )
        ...
        if permission:
            logger.info(
                f"✅ Permissions found: can_import={permission.can_import}, max_size={permission.max_file_size_mb}MB"
            )
            if not permission.can_import:
                logger.warning(
                    f"❌ Import permission denied for user {current_user.id} on table {table_name}"
                )
                raise HTTPException(
        ...
        logger.info(
            f"📏 Validating file size: {file.size} bytes (max: {max_file_size_mb}MB)"
        )
        ...
        try:
            ...
            logger.info(
                f"✅ File parsed successfully: {parsed_data.get('total_rows', 0)} rows, {len(parsed_data.get('headers', []))} columns"
            )
        ...
        try:
            import_job = ImportJob(
                ...
                or (permission.requires_approval if permission else False),
        ...
        logger.info(
            f"📈 Summary: {parsed_data.get('total_rows', 0)} rows, {len(parsed_data.get('headers', []))} columns, Status: {ImportStatus.PARSED}"
        )
...
async def parse_import_file(
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"📋 File preview parsed: {file.filename} ({parsed_data['total_rows']} rows)"
            )
        except Exception as e:
...
async def validate_import_file(
    ...
    try:
        logger.info(
            f"🔍 Starting file validation: {file.filename} for table: {table_name}"
        )
        ...
        try:
            ...
            mappings = json.loads(field_mappings)
            logger.info(
                f"✅ Options parsed: format={options.format}, mappings={len(mappings)}"
            )
        except Exception as e:
        ...
        if permission and not permission.can_import:
            logger.warning(
                f"❌ Validation permission denied for user {current_user.id} on table {table_name}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        try:
            ...
            parsed_data = _parse_uploaded_file(file_content, file.filename, options)
            logger.info(
                f"✅ File parsed: {parsed_data.get('total_rows', 0)} rows, {len(parsed_data.get('headers', []))} columns"
            )
        except Exception as e:
            logger.error(f"❌ Failed to parse file: {e}")
            raise HTTPException(
        ...
        try:
            ...
            logger.info(
                f"✅ File validation completed: {validation_result['valid_rows']}/{validation_result['total_rows']} valid rows"
            )
        except Exception as e:
...
async def validate_import_data(
    ...
    try:
        ...
        if (
            not import_job.import_results
            or "parsed_data" not in import_job.import_results
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def start_import(
    ...
    try:
        ...
        if not import_job.validation_results or not import_job.validation_results.get(
            "is_valid"
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def list_import_jobs(
    ...
    try:
        ...
        jobs = (
            query.order_by(desc(ImportJob.created_at)).offset(skip).limit(limit).all()
        )
...
async def get_import_progress(
    ...
    try:
        ...
        if import_job.created_by != current_user.id and not current_user.is_superuser:
            logger.warning(
                f"❌ Access denied to job {job_id} for user {current_user.id}"
            )
        ...
        current_progress = progress_map.get(
            import_job.status, {"percent": 0, "message": "Unknown status"}
        )
        ...
        details = {
            ...
            "updated_at": (
                import_job.updated_at.isoformat() if import_job.updated_at else None
            ),
            "logs": [],
        }
        ...
        if (
            ...
            and "validation_results" in import_job.import_results
        ):
        ...
        try:
            ...
            recent_logs = (
                ...
                .filter(ImportAuditLog.import_job_id == import_job.id)
                .order_by(ImportAuditLog.timestamp.desc())
                .limit(10)
                .all()
            )
            ...
            details["logs"] = [
                {
                    ...
                    "message": (
                        log.event_data.get(
                            "message", f"{log.event_type.replace('_', ' ').title()}"
                        )
                        if log.event_data
                        else f"{log.event_type.replace('_', ' ').title()}"
                    ),
                    "level": (
                        log.event_data.get("level", "info")
                        if log.event_data
                        else "info"
                    ),
                    "stage": log.event_data.get("upload_stage")
                    or log.event_data.get("validation_stage")
                    or log.event_data.get("import_stage")
                    or log.event_data.get("failure_stage", ""),
        ...
        logger.info(
            f"✅ Progress retrieved for job {job_id}: {current_progress['percent']}%"
        )
        return details
...
async def create_export_job(
    ...
    try:
        ...
        logger.info(
            f"Export request by user {current_user.id} ({current_user.email}) for table {export_request.table_name}"
        )

        # Check permissions - use default permissions if none found
        permission = _check_export_permission(
            db, current_user.id, export_request.table_name
        )
        if permission and not permission.can_export:
            logger.warning(
                f"Export permission denied for user {current_user.id} on table {export_request.table_name}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        if (
            not export_request.selected_columns
            or len(export_request.selected_columns) == 0
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        file_content, processed_rows = _generate_export_file(
            export_request, current_user, db
        )
        ...
        file_content_b64 = base64.b64encode(file_content.encode("utf-8")).decode(
            "utf-8"
        )
        ...
        logger.error(
            f"Export job creation failed: {e}\nFull traceback: {error_details}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    try:
        ...
        return {
            ...
            "export_job": {
                ...
                "created_at": (
                    export_job.created_at.isoformat() if export_job.created_at else None
                ),
            },
        }
...
async def download_export(
    ...
    try:
        ...
        logger.info(
            f"Download request - Job ID: {job_id}, Current User: {current_user.id}, Job Creator: {export_job.created_by}"
        )

        # Verify user has permission to download this export
        if export_job.created_by != current_user.id:
            logger.warning(
                f"Access denied - User {current_user.id} trying to download job created by {export_job.created_by}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Access denied - you can only download your own exports",
            )
        ...
        if (
            not export_job.export_results
            or "file_content" not in export_job.export_results
        ):
            raise HTTPException(
        ...
        logger.info(
            f"Serving download for job {job_id} - filename: {filename}, size: {export_job.file_size} bytes"
        )
...
async def list_export_jobs(
    ...
    try:
        ...
        jobs = (
            query.order_by(desc(ExportJob.created_at)).offset(skip).limit(limit).all()
        )
...
def _parse_uploaded_file(file_content: bytes, filename: str, options) -> dict:
    ...
    logger.info(
        f"📊 File details: {len(file_content)} bytes, format: {getattr(options, 'format', 'auto')}"
    )
    ...
    try:
        ...
        if isinstance(file_content, bytes):
            try:
                ...
                logger.info(
                    f"✅ File decoded successfully: {len(content_str)} characters"
                )
        ...
        if file_format == "csv":
            ...
            logger.info(
                f"✅ CSV parsing completed: {parsed_data.get('total_rows', 0)} rows"
            )
            return parsed_data
        elif file_format == "json":
            parsed_data = _parse_json_content(content_str, options)
            logger.info(
                f"✅ JSON parsing completed: {parsed_data.get('total_rows', 0)} rows"
            )
            return parsed_data
        else:
                ...
                if not column.nullable and column.default is None:
                    if column.name in headers:
                        ...
                        if not value or str(value).strip() == "":
                            row_errors.append(
                                f"Row {i+1}: Required field '{column.name}' is empty"
                            )
...
def _perform_data_import(import_job, db: Session, current_user) -> dict:
    ...
    try:
            ...
            try:
                ...
                for header in headers:
                    if header in table_columns:
                        ...
                        converted_value = _convert_value_for_column(
                            value, table_columns[header]
                        )
        ...
        logger.info(
            f"Import completed: {valid_rows} valid, {error_rows} errors out of {processed_rows} processed"
        )
...
def _convert_value_for_column(value, column):
    ...
    try:
        ...
        logger.warning(
            f"Could not convert value '{value}' for column {column.name}: {e}"
        )
        return str(value)
...
def _generate_export_file(export_request, current_user, db: Session) -> tuple[str, int]:
    ...
    try:
        ...
        real_data = _get_real_table_data(
            db, export_request.table_name, export_request.selected_columns
        )
        ...
        if (
            hasattr(export_request.export_options, "search_term")
            and export_request.export_options.search_term
        ):
...
def _get_real_table_data(db: Session, table_name: str, selected_columns: list) -> list:
    ...
    try:
        ...
        if not model_class:
            logger.warning(
                f"Model not found for table {table_name}, falling back to sample data"
            )
...
def _get_real_table_data_paginated(
    db: Session, table_name: str, limit: int, offset: int, search: Optional[str] = None
) -> dict:
                ...
                if hasattr(column.type, "python_type"):
                    if column.type.python_type == str:
                        search_conditions.append(
                            func.lower(getattr(model_class, column.name)).like(
                                search_term
                            )
                        )
...
def _get_sample_paginated_data(
    table_name: str, limit: int, offset: int, search: Optional[str] = None
) -> dict:
    ...
    sample_data = [
        ...
        {
            "id": 3,
            "name": f"Sample {table_name} 3",
            "status": "completed",
            "created_at": "2024-01-03T10:00:00",
        },
        {
            "id": 4,
            "name": f"Sample {table_name} 4",
            "status": "active",
            "created_at": "2024-01-04T10:00:00",
        },
        {
            "id": 5,
            "name": f"Sample {table_name} 5",
            "status": "archived",
    ...
    if search:
        ...
        sample_data = [
            ...
            or search_lower in item["status"].lower()
        ]
    ...
    for i in range(1, 51):  # Generate 50 sample rows
        ...
        for col in selected_columns:
            if col.lower() == "id":
                ...
                row[col] = (datetime.utcnow() - timedelta(days=i)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
    ...
    for filter_item in filters:
        ...
        if operator == "equals":
            ...
            filtered_data = [
                row
                for row in filtered_data
                if str(row.get(column, "")).lower().startswith(value.lower())
            ]
...
def _check_import_permission(
    db: Session, user_id: int, table_name: str
) -> Optional[ImportPermission]:
...
def _check_export_permission(
    db: Session, user_id: int, table_name: str
) -> Optional[ExportPermission]:
    """Check export permission for user and table"""

    return (
        db.query(ExportPermission)
        .filter(
            and_(
...
def _log_export_audit(
    db: Session, job_id: int, user_id: int, event_type: str, event_data: Dict[str, Any]
):
...
def get_available_tables(
    ...
    try:
        ...
        return {"tables": available_tables, "total_count": len(available_tables)}
...
def get_table_data(
    ...
    try:
        ...
        real_data = _get_real_table_data_paginated(
            db, table_name, limit, offset, search
        )
