...
def get_audit_trails(
    ...
    changed_fields_contain: Optional[str] = Query(
        None, description="Filter by field name in changed fields"
    ),
) -> Any:
    ...
    if changed_fields_contain is not None:
        query = query.filter(
            AuditTrail.changed_fields.ilike(f"%{changed_fields_contain}%")
        )
...
@router.get(
    "/entity/{entity_type}/{entity_id}", response_model=List[AuditTrailResponse]
)
def get_entity_audit_history(
    *,
    ...
    audit_trails = (
        ...
        .filter(
            and_(
                AuditTrail.entity_type == entity_type, AuditTrail.entity_id == entity_id
            )
        )
...
def get_audit_trail_comparison(
    *,
    ...
    try:
        old_values = (
            json.loads(audit_trail.old_values) if audit_trail.old_values else {}
        )
        new_values = (
            json.loads(audit_trail.new_values) if audit_trail.new_values else {}
        )
        changed_fields = (
            json.loads(audit_trail.changed_fields) if audit_trail.changed_fields else []
        )
...
def get_audit_trail_stats(
    ...
    for audit in audit_trails:
        ...
        changes_by_entity_type[entity_key] = (
            changes_by_entity_type.get(entity_key, 0) + 1
        )
...
def bulk_delete_audit_trails(
    *,
    ...
    older_than_days: int = Query(
        ..., ge=1, description="Delete audit trails older than this many days"
    ),
    entity_type: Optional[str] = Query(
        None, description="Only delete audit trails of this entity type"
    ),
    operation: Optional[str] = Query(
        None, description="Only delete audit trails of this operation"
    ),
) -> Any:
