...
def get_activity_log_stats(
    ...
    for activity in activities:
        ...
        activities_by_entity_type[entity_key] = (
            activities_by_entity_type.get(entity_key, 0) + 1
        )
...
def bulk_delete_activity_logs(
    *,
    ...
    older_than_days: int = Query(
        ..., ge=1, description="Delete logs older than this many days"
    ),
    level: Optional[ActivityLevel] = Query(
        None, description="Only delete logs of this level"
    ),
    entity_type: Optional[str] = Query(
        None, description="Only delete logs of this entity type"
    ),
) -> Any:
