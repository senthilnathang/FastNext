...
from app.schemas.workflow import WorkflowType as WorkflowTypeSchema, WorkflowTypeCreate, WorkflowTypeUpdate
...
def read_workflow_types(
    ...
    try:
        ...
        print(f"Error fetching workflow types: {e}")
...
def create_workflow_type(
    *,
    ...
    if not PermissionService.check_permission(db, current_user.id, "create", "workflow_type"):
        raise HTTPException(
    ...
    existing_type = db.query(WorkflowType).filter(WorkflowType.name == workflow_type_in.name).first()
    if existing_type:
        raise HTTPException(
...
def read_workflow_type(
    *,
    ...
    workflow_type = db.query(WorkflowType).filter(WorkflowType.id == workflow_type_id).first()
...
def update_workflow_type(
    *,
    ...
    if not PermissionService.check_permission(db, current_user.id, "update", "workflow_type"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
def delete_workflow_type(
    *,
    ...
    if not PermissionService.check_permission(db, current_user.id, "delete", "workflow_type"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
