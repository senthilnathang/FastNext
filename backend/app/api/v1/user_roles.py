...
def read_user_roles(
    *,
    ...
    if user_id != current_user.id:
        if not PermissionService.check_resource_permission(db, current_user.id, "read", "user"):
            raise HTTPException(
...
def assign_role_to_user(
    *,
    ...
    if not PermissionService.check_resource_permission(db, current_user.id, "create", "user"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    *,
    ...
    if user_role.user_id != current_user.id:
        if not PermissionService.check_resource_permission(db, current_user.id, "read", "user"):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="User role access denied"
            )
...
def update_user_role(
    *,
    ...
    if not PermissionService.check_resource_permission(db, current_user.id, "update", "user"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
def remove_role_from_user(
    *,
    ...
    if not PermissionService.check_resource_permission(db, current_user.id, "delete", "user"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
