...
from app.core.performance_monitoring import MetricType, performance_collector
...
class OptimizedUserService:
    ...
    async def get_user_analytics(self) -> UserAnalyticsResponse:
        ...
        results = await task_manager.execute_concurrent(
            analytics_tasks, max_concurrent=5
        )
    ...
    async def _get_users_by_role(self) -> dict:
        ...
        result = await self.db.execute(
            select(Role.name, func.count(User.id)).join(User.roles).group_by(Role.name)
        )
...
async def get_optimized_user(
    ...
    # Record metrics
    performance_collector.add_metric(
        MetricType.RESPONSE_TIME, execution_time, labels={"endpoint": "get_user_by_id"}
    )
...
async def get_user_analytics(
    ...
    background_tasks.add_task(update_analytics_cache, analytics.dict(), execution_time)
...
@router.get("/health-check", summary="Health check with performance metrics")
async def optimized_health_check() -> dict:
    ...
    optimization_metrics = {
        ...
        "performance_collector": {
            "total_requests": performance_collector.stats.get("get_users", {}).get(
                "total_queries", 0
            ),
    ...
    return {
        ...
        "components": {
            "database": (
                db_health if not isinstance(db_health, Exception) else str(db_health)
            ),
            "cache": (
                cache_health
                if not isinstance(cache_health, Exception)
                else str(cache_health)
            ),
            "async_tasks": (
                async_health
                if not isinstance(async_health, Exception)
                else str(async_health)
            ),
        },
...
@router.post("/users/batch-process", summary="Batch process users with optimization")
async def batch_process_users(
    ...
    if len(user_ids) > 1000:
        raise HTTPException(
            status_code=400, detail="Batch size too large. Maximum 1000 users."
        )
    ...
    for i in range(0, len(user_ids), batch_size):
        ...
        background_tasks.add_task(
            process_user_batch, batch, batch_number=i // batch_size + 1
        )
...
async def process_user_batch(user_ids: List[int], batch_number: int):
    ...
    results = await task_manager.execute_concurrent(tasks, max_concurrent=10)
    ...
    logger.info(
        ...
        extra={"successful": successful, "failed": failed, "total": len(user_ids)},
    )
