...
from app.schemas.rls import (
    ...
    RLSAccessCheckRequest,
    RLSAccessCheckResponse,
    RLSAuditLogResponse,
    RLSContextResponse,
    RLSPolicyCreate,
    RLSPolicyResponse,
    RLSPolicyUpdate,
    RLSRuleAssignmentCreate,
    RLSRuleAssignmentResponse,
)
...
async def list_rls_policies(
    *,
    ...
    query = query.order_by(
        RowLevelSecurityPolicy.priority.desc(), RowLevelSecurityPolicy.created_at.desc()
    )
...
@router.get(
    "/policies/{policy_id}", response_model=RLSPolicyResponse, tags=["RLS Policies"]
)
@require_rls_access(RLSEntityType.PERMISSION, RLSAction.SELECT)
async def get_rls_policy(
    *,
    ...
    policy = (
        db.query(RowLevelSecurityPolicy)
        .filter(RowLevelSecurityPolicy.id == policy_id)
        .first()
    )

    if not policy:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="RLS policy not found"
        )
...
@router.post(
    "/rule-assignments", response_model=RLSRuleAssignmentResponse, tags=["RLS Rules"]
)
@require_rls_access(RLSEntityType.PERMISSION, RLSAction.INSERT)
...
@router.get(
    "/rule-assignments",
    response_model=List[RLSRuleAssignmentResponse],
    tags=["RLS Rules"],
)
@require_rls_access(RLSEntityType.PERMISSION, RLSAction.SELECT)
async def list_rule_assignments(
    *,
    ...
    query = db.query(RLSRuleAssignment).filter(RLSRuleAssignment.is_active == True)
    ...
    return [
        RLSRuleAssignmentResponse.from_orm(assignment) for assignment in assignments
    ]
...
async def get_current_context(
    *, current_user: User = Depends(get_current_active_user), request: Request
) -> Any:
    ...
    if not context:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="RLS context not found"
        )
...
@router.post(
    "/context/refresh", response_model=RLSContextResponse, tags=["RLS Context"]
)
    ...
    *,
    ...
    try:
        ...
        context = rls_service.create_context(
            user=current_user, session_id=session_id, request=request
        )
...
@router.post(
    "/check-access", response_model=RLSAccessCheckResponse, tags=["RLS Access"]
)
...
async def get_audit_stats(
    *,
    ...
    total_attempts = (
        db.query(RLSAuditLog).filter(RLSAuditLog.created_at >= from_date).count()
    )

    # Granted vs denied
    granted_count = (
        db.query(RLSAuditLog)
        .filter(RLSAuditLog.created_at >= from_date, RLSAuditLog.access_granted == True)
        .count()
    )
    ...
    return {
        ...
        "success_rate": (
            (granted_count / total_attempts * 100) if total_attempts > 0 else 0
        ),
        "top_denied_reasons": [
            {"reason": reason, "count": count} for reason, count in denied_reasons
        ],
        "entity_type_stats": [
...
@router.post(
    "/organizations", response_model=OrganizationResponse, tags=["Organizations"]
)
...
@router.get(
    "/organizations", response_model=List[OrganizationResponse], tags=["Organizations"]
)
