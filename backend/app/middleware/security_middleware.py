...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        logger.info(
            f"SecurityMiddleware: {request.method} {request.url.path} from {client_ip}"
        )
        ...
        try:
            ...
            self._log_request(
                request, response, client_ip, user_agent, process_time, request_id
            )
            ...
            self._log_security_event(
                request, client_ip, user_agent, str(e.detail), request_id
            )
            raise
        except Exception as e:
            # Log unexpected errors
            logger.error(
                f"Unexpected error in security middleware: {e}",
                extra={
...
class SessionExpirationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if hasattr(request.state, "session_timeout"):
            response.headers["X-Session-Timeout"] = str(
                self.session_timeout_minutes * 60
            )
            response.headers["X-Session-Warning"] = str(
                (self.session_timeout_minutes - 5) * 60
            )
