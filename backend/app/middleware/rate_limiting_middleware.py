...
try:
    ...
    logger.warning(
        "Redis not available for distributed rate limiting, using in-memory fallback"
    )
...
class RateLimitingMiddleware(BaseHTTPMiddleware):
    ...
    def setup_rate_limits(self):
        ...
        self.endpoint_limits = {
            "/api/v1/auth/login": {"per_minute": 5, "per_hour": 20, "per_day": 100},
            "/api/v1/auth/register": {"per_minute": 2, "per_hour": 10, "per_day": 50},
            "/api/v1/auth/reset-password": {
                "per_minute": 1,
                "per_hour": 5,
                "per_day": 20,
            },
            "/api/v1/users": {"per_second": 5, "per_minute": 50, "per_hour": 500},
            "/api/v1/projects": {"per_second": 20, "per_minute": 200, "per_hour": 2000},
            # File upload endpoints
            "/api/v1/upload": {"per_minute": 10, "per_hour": 100},
            # Search endpoints
            "/api/v1/search": {"per_second": 5, "per_minute": 100, "per_hour": 1000},
        }
        ...
        self.security_limits = {
            "failed_auth_attempts": {"per_minute": 5, "per_hour": 20, "per_day": 100},
            "malicious_requests": {"per_minute": 1, "per_hour": 5, "per_day": 10},
            "suspicious_activity": {"per_hour": 10, "per_day": 50},
        }
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        try:
            ...
            limit_result = await self._check_rate_limits(
                client_id, endpoint, user_info, request
            )

            if not limit_result.allowed:
                await self._log_rate_limit_exceeded(
                    client_id, endpoint, limit_result, request
                )
    ...
    async def _check_rate_limits(
        self, client_id: str, endpoint: str, user_info: Dict[str, Any], request: Request
    ) -> "RateLimitResult":
        ...
        # All limits passed
        remaining = min(
            limit - await self._get_current_usage(client_id, endpoint, window)
            for window, limit in limits.items()
        )
    ...
    def _get_applicable_limits(
        self, endpoint: str, user_info: Dict[str, Any]
    ) -> Dict[str, int]:
    ...
    async def _get_current_usage(
        self, client_id: str, endpoint: str, window: str
    ) -> int:
    ...
    async def _update_counters(
        ...
        for window in ["per_second", "per_minute", "per_hour", "per_day"]:
            if redis_available:
                await self._update_redis_counter(
                    client_id, endpoint, window, current_time
                )
            else:
    ...
    async def _update_redis_counter(
        self, client_id: str, endpoint: str, window: str, timestamp: float
    ):
    ...
    async def _track_performance_metrics(
        self, client_id: str, endpoint: str, process_time: float
    ):
    ...
    def _add_rate_limit_headers(
        self, response: Response, limit_result: "RateLimitResult"
    ):
    ...
    def _create_rate_limit_response(
        self, limit_result: "RateLimitResult"
    ) -> JSONResponse:
        ...
        try:
            ...
            if response_times:
                ...
                if avg_response_time > self.performance_threshold:
                    ...
                    logger.warning(
                        f"Reducing rate limits due to high response time: {avg_response_time:.2f}s"
                    )
                    ...
                    logger.info(
                        f"Increasing rate limits due to good performance: {avg_response_time:.2f}s"
                    )
