...
class ResponseOptimizer:
        ...
        try:
            ...
            if "br" in accept_encoding:
                compressed = brotli.compress(
                    content, 
                    quality=self.config.compression_level.value
                )
    ...
    async def _apply_caching_headers(
        ...
        if path.startswith("/api/"):
            ...
            response.headers["cache-control"] = f"public, max-age={self.config.cache_ttl}"
...
class OptimizationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if self.config.optimization_strategy != OptimizationStrategy.DISABLED:
            ...
            if optimized_size < original_size:
                ...
                self.performance_metrics["compression_ratio"] = (
                    (self.performance_metrics["compression_ratio"] * 
                     self.performance_metrics["optimized_requests"] + compression_ratio) /
        ...
        if self.config.enable_response_caching:
            try:
                ...
                if response.headers.get("content-type", "").startswith("application/json"):
                    ...
                    await self.cache_middleware.cache_response(request, response, json_content)
                else:
                    await self.cache_middleware.cache_response(request, response, content.decode('utf-8'))
            except Exception as e:
        ...
        response.headers["x-optimized"] = "true" if self.performance_metrics["optimized_requests"] > 0 else "false"
...
class ConditionalOptimizationMiddleware(OptimizationMiddleware):
    ...
    def __init__(
        ...
        self.optimization_rules = optimization_rules or self._default_optimization_rules()
    ...
    def _default_optimization_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "slow_connection_indicators": [
                "2G", "slow", "save-data"
            ],
            "priority_paths": [
                "/api/users", "/api/dashboard", "/api/analytics"
            ],
            "aggressive_optimization_paths": [
                "/api/reports", "/api/export"
            ]
        }
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            is_aggressive_path = any(
                ...
                for aggressive_path in self.optimization_rules["aggressive_optimization_paths"]
            )
...
def setup_optimization_middleware(
    ...
    logger.info(f"Optimization middleware configured with strategy: {config.optimization_strategy.value}")
