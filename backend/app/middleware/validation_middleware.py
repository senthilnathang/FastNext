...
class ValidationMiddleware(BaseHTTPMiddleware):
    ...
    async def _validate_headers(self, request: Request) -> "ValidationResult":
        ...
        try:
            ...
            for header_name, header_value in request.headers.items():
                ...
                if len(header_value) > self.max_lengths["header"]:
                    ...
                    return ValidationResult(
                        False, f"Header {header_name} too long", "HEADER_TOO_LONG"
                    )
            ...
            if request.method in ["POST", "PUT", "PATCH"]:
                ...
                if not content_type:
                    return ValidationResult(
                        False, "Content-Type header required", "MISSING_CONTENT_TYPE"
                    )
            ...
            return ValidationResult(
                False, "Header validation failed", "HEADER_VALIDATION_ERROR"
            )
        ...
        try:
            for param_name, param_value in request.query_params.items():
                ...
                if len(param_value) > self.max_lengths["query_param"]:
                    ...
                    return ValidationResult(
                        False,
                        f"Query parameter {param_name} too long",
                        "QUERY_PARAM_TOO_LONG",
                    )
                ...
                if (
                    param_name.lower() in ["id", "user_id", "project_id"]
                    and not param_value.isdigit()
                ):
                    if len(param_value) > 50 or self._contains_sql_patterns(
                        param_value
                    ):
                        await self._log_validation_error(
                            "suspicious_id_param",
                        ...
                        return ValidationResult(
                            False,
                            f"Invalid ID format: {param_name}",
                            "INVALID_ID_FORMAT",
                        )
            ...
            return ValidationResult(
                False, "Query parameter validation failed", "QUERY_VALIDATION_ERROR"
            )
    ...
    async def _validate_request_body(self, request: Request) -> "ValidationResult":
        ...
        try:
                ...
                try:
                    ...
                    if body:
                        ...
                        if len(body_str) > self.max_lengths["json"]:
                            return ValidationResult(
                                False, "JSON payload too large", "JSON_TOO_LARGE"
                            )
                    ...
                    return ValidationResult(
                        False, "Invalid JSON format", "INVALID_JSON"
                    )
                except UnicodeDecodeError:
                    return ValidationResult(
                        False, "Invalid character encoding", "INVALID_ENCODING"
                    )
            ...
            return ValidationResult(
                False, "Request body validation failed", "BODY_VALIDATION_ERROR"
            )
        ...
        try:
            if isinstance(data, str):
                ...
                if len(data) > self.max_lengths["text"]:
                    return ValidationResult(
                        False, f"String too long at {path}", "STRING_TOO_LONG"
                    )

                # Check for malicious patterns
                if self._contains_malicious_patterns(data):
                    return ValidationResult(
                        False, f"Malicious content at {path}", "MALICIOUS_JSON_CONTENT"
                    )
                ...
                for key, value in data.items():
                    ...
                    if not isinstance(key, str) or len(key) > 100:
                        return ValidationResult(
                            False, f"Invalid key at {path}", "INVALID_JSON_KEY"
                        )
                ...
                if len(data) > 1000:  # Maximum array size
                    return ValidationResult(
                        False, f"Array too large at {path}", "ARRAY_TOO_LARGE"
                    )
            ...
            return ValidationResult(
                False, "JSON content validation failed", "JSON_VALIDATION_ERROR"
            )
    ...
    async def _validate_file_upload(self, request: Request) -> "ValidationResult":
        ...
        try:
            ...
            return ValidationResult(
                False, "File upload validation failed", "FILE_VALIDATION_ERROR"
            )
    ...
    async def _log_validation_error(
        self, error_type: str, message: str, request: Request
    ):
...
class EnhancedRequestValidator:
    ...
    def validate_json_body(
        body: bytes, schema_class: Type[BaseModel], strict: bool = True
    ) -> BaseModel:
    ...
    def validate_file_upload_enhanced(
        ...
        if not result["is_valid"]:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail={"message": "File validation failed", "error": result["error"]},
            )
            ...
            async def wrapper(*args, **kwargs):
                if "validated_data" in kwargs:
                    ...
                    if hasattr(data, "password"):
                        password_result = (
                            EnhancedRequestValidator.validate_password_field(
                                data.password
                            )
                        )
                ...
                if "validated_data" in kwargs:
                    ...
                    for field in html_fields:
                        if hasattr(data, field):
                            sanitized_content = (
                                EnhancedRequestValidator.sanitize_html_content(
                                    getattr(data, field)
                                )
                            )
...
async def validate_and_parse_request_body(
    request: Request, schema_class: Type[BaseModel]
) -> BaseModel:
...
def validate_pagination_params_enhanced(
    ...
    if page < 1:
        raise create_validation_error_response(
            "Page must be greater than 0", error_code="INVALID_PAGE"
        )

    if limit < 1 or limit > max_limit:
        raise create_validation_error_response(
            f"Limit must be between 1 and {max_limit}", error_code="INVALID_LIMIT"
        )

    # Validate sort_order
    if sort_order not in ["asc", "desc"]:
        raise create_validation_error_response(
            "Sort order must be 'asc' or 'desc'", error_code="INVALID_SORT_ORDER"
        )
