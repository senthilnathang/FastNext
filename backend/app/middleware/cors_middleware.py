...
class SecurityCORSMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        self.strict_origin_validation = self.security_config.get(
    ...
    def setup_security_policies(self):
        ...
        self.blocked_origins = self.security_config.get(
            "blocked_origins",
            [
                "null",
                "file://",
    ...
    async def _handle_preflight(
        ...
        if requested_headers:
            ...
            invalid_headers = [
                h for h in headers_list if not self._is_header_allowed(h)
            ]
            ...
            if invalid_headers:
                ...
                return PlainTextResponse("Headers not allowed", status_code=400)
    ...
    async def _add_cors_headers(
        self, response: Response, request: Request, origin: Optional[str]
    ):
        ...
        try:
            ...
            if self.enforce_https and parsed.scheme != "https":
                ...
                if not (
                    ...
                    and not self.strict_origin_validation
                ):
                    return False
...
def create_cors_middleware(
    preset: str = "development", custom_config: Optional[Dict] = None
):
