...
class EnhancedEventLoggingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        self.log_level_threshold = getattr(
            ActivityLevel, log_level_threshold.upper(), ActivityLevel.INFO
        )
        ...
        self.exclude_paths = exclude_paths or {
            ...
            "/events",
            "/events/",
            "/api/v1/events",  # Prevent recursive logging
        }
    ...
    async def _log_enhanced_event(
        ...
        risk_score = self._calculate_risk_score(
            request, response, user_info, is_sensitive
        )
    ...
    def _determine_log_level(
        self, status_code: int, is_sensitive: bool = False
    ) -> ActivityLevel:
    ...
    def _generate_event_description(
        self, request: Request, response: Response, user_info: Dict[str, Any]
    ) -> str:
    ...
    def _prepare_metadata(
        ...
        metadata = {
            ...
            "user_agent_type": self._classify_user_agent(
    ...
    def _generate_tags(
        self, request: Request, response: Response, is_sensitive: bool
    ) -> list:
    ...
    def _classify_user_agent(self, user_agent: str) -> str:
        ...
        if (
            ...
            "postman" in user_agent_lower
            or "curl" in user_agent_lower
            or "httpie" in user_agent_lower
        ):
            return "api_client"
        else:
            return "unknown"
...
class AuthenticationEventMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if not any(
            request.url.path.startswith(endpoint) for endpoint in self.auth_endpoints
        ):
            return await call_next(request)
