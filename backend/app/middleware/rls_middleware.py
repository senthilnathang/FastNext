...
class RLSMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self, app, enable_audit: bool = True, exclude_paths: Optional[List[str]] = None
    ):
        super().__init__(app)
...
def rls_filter_query(entity_type: RLSEntityType, action: RLSAction = RLSAction.SELECT):
    """
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if hasattr(result, "filter"):  # It's a query object
                ...
                if current_user and hasattr(request.state, "rls_service"):
                    rls_service = request.state.rls_service
                    session_id = getattr(request.state, "rls_context", {}).get(
                        "session_id"
                    )
...
def rls_audit_action(
    entity_type: RLSEntityType, action: RLSAction, description: Optional[str] = None
):
    """
