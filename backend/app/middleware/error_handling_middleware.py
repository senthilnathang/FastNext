...
class ErrorHandlingMiddleware(BaseHTTPMiddleware):
    ...
    async def _process_with_timeout(
        self, request: Request, call_next, timeout: int = 30
    ) -> Response:
        """Process request with timeout protection"""
        try:
    ...
    async def _handle_error(
        self, exception: Exception, request: Request, request_id: str, start_time: float
    ) -> JSONResponse:
        """Main error handler"""
        ...
        category_config = self.error_categories.get(
            category, self.error_categories["server"]
        )
        ...
        if category in self.recovery_strategies:
            try:
                ...
                if recovery_result.get("recovered"):
                    ...
                    logger.info(
                        f"Error recovery successful for {category}: {recovery_result}"
                    )
        ...
        if isinstance(exception, HTTPException):
            ...
            return await self._handle_fastnext_exception(
                exception, request, request_id, category_config
            )
        else:
    ...
    def _categorize_error(self, exception: Exception) -> str:
        ...
        for category, config in self.error_categories.items():
            if any(
                isinstance(exception, exc_type) for exc_type in config["exceptions"]
            ):
                return category
    ...
    async def _handle_fastnext_exception(
        self,
        exception: FastNextException,
        request: Request,
        request_id: str,
    ...
    def _create_error_response(
        self, message: str, error_code: str, status_code: int, request_id: str
    ) -> JSONResponse:
    ...
    async def _recover_external_service_error(
        ...
        try:
            ...
            return {
                "recovered": False,
                "reason": "External service recovery not implemented",
            }
        except Exception as e:
            return {"recovered": False, "error": str(e)}
...
class ErrorRateTracker:
    ...
    async def _check_error_rate(
        self, client_ip: str, error_type: str, current_time: int
    ):
...
class ErrorReporter:
    ...
    async def report_error(self, error_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"Error reported to monitoring service: {error_data.get('error_type')}"
            )
        except Exception as e:
