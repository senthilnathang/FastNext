    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if not token:
            return self._create_auth_error_response(
                "Authentication required", "MISSING_TOKEN"
            )
        ...
        try:
            ...
            if await self._is_token_blacklisted(token):
                return self._create_auth_error_response(
                    "Token has been revoked", "REVOKED_TOKEN"
                )
            ...
            if not await self._check_user_status(user_data, request):
                return self._create_auth_error_response(
                    "Account disabled or suspended", "ACCOUNT_DISABLED"
                )
    ...
    async def _check_user_status(
        ...
        if not user_data.get("is_active", True):
            await self._log_auth_failure(
                "account_disabled", "User account is disabled", request, user_data
            )
            return False
    ...
    async def _detect_suspicious_activity(
        ...
        try:
            ...
            if len(current_ips) > 5:  # More than 5 different IPs in time window
                logger.warning(
                    f"User {user_id} accessing from multiple IPs: {current_ips}"
                )
                return True
            ...
            if request_count > 100:  # More than 100 requests per minute
                logger.warning(
                    f"User {user_id} making too many requests: {request_count}/min"
                )
                return True
    ...
    async def _check_user_rate_limit(self, user_id: str, request: Request) -> bool:
        ...
        try:
            ...
            if current_requests > 1000:
                logger.warning(
                    f"User {user_id} exceeded rate limit: {current_requests}/hour"
                )
                return False
