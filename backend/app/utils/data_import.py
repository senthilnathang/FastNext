...
from typing import IO, Any, AsyncGenerator, BinaryIO, Dict, List, Optional, Union
...
from app.models.data_import_export import DataFormat, ImportJob, ImportStatus
...
from sqlalchemy import Column, MetaData, Table, func, inspect, select, text
...
class DataImporter:
    ...
    async def parse_file(
        self, file: UploadFile, import_options: ImportOptionsSchema
    ) -> Dict[str, Any]:
    ...
    async def _parse_csv(
        self, content: bytes, options: ImportOptionsSchema
    ) -> Dict[str, Any]:
    ...
    async def validate_data(
        ...
        try:
            ...
            mapping_lookup = {
                mapping.source_column: mapping for mapping in field_mappings
            }
            ...
            for row_idx, row in enumerate(data, 1):
                ...
                for target_col, col_info in table_schema.items():
                    if (
                        col_info.get("nullable") is False
                        and col_info.get("default") is None
                    ):
                        ...
                        for mapping in field_mappings:
                            if (
                                ...
                                and mapping.source_column in row
                            ):
                                if (
                                    row[mapping.source_column] is not None
                                    and str(row[mapping.source_column]).strip()
                                ):
                                    mapped = True
                                    break
    ...
    async def _get_table_schema(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ImportError(
                f"Could not get schema for table '{table_name}': {str(e)}"
            )
    ...
    async def _insert_chunk(
        self, table_name: str, rows: List[Dict[str, Any]], options: ImportOptionsSchema
    ):
    ...
    async def _upsert_row(self, table_name: str, row: Dict[str, Any]):
        ...
        update_clause = ", ".join(
            [f"{k} = EXCLUDED.{k}" for k in row.keys() if k != "id"]
        )
