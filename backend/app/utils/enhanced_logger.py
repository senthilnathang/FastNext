...
class EnhancedLogger:
    ...
    def get_logs_from_file(
        self, date: Optional[str] = None, lines: int = 500
    ) -> Dict[str, Any]:
        """
    ...
    def get_event_statistics(self, db: Session, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_events = (
                base_query.filter(ActivityLog.level == ActivityLevel.CRITICAL)
            ...
            return {
                ...
                "totals": {
                    ...
                    "users": base_query.filter(ActivityLog.user_id.isnot(None))
                    .distinct(ActivityLog.user_id)
                    .count(),
                },
                "byLevel": level_stats,
...
def log_authentication_event(
    ...
    description = (
        f"User {username} {action.value} {'successful' if success else 'failed'}"
    )
...
def log_api_call(
    ...
    return enhanced_logger.log_event(
        ...
        request=request,
        response_time_ms=response_time_ms,
        metadata={"status_code": status_code, "response_time_ms": response_time_ms},
    )
