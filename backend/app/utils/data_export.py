...
class DataExporter:
    ...
    async def export_table(
        ...
        try:
            ...
            return await self._stream_export(
                table_name, format, filters, columns, limit
            )
        ...
        if filters:
            ...
            for column, value in filters.items():
                if isinstance(value, dict) and "operator" in value:
                    ...
                    if op == "in":
                        placeholders = ",".join(
                            [f":filter_{column}_{i}" for i in range(len(val))]
                        )
    ...
    async def _stream_export(
        ...
        async def generate_export():
            async for chunk in self._export_table_chunks(
                table_name, format, filters, columns, limit
            ):
                yield chunk
    ...
    async def _export_data_chunks(
        self, query: str, format: ExportFormat, parameters: Dict[str, Any]
    ) -> AsyncGenerator[bytes, None]:
    ...
    def _get_media_type_and_filename(
        self, format: ExportFormat, base_name: str
    ) -> tuple:
        ...
        return format_config.get(
            format, ("application/octet-stream", f"{base_name}_{timestamp}.dat")
        )
    ...
    async def _start_background_query_export(
        ...
        background_tasks.add_task(
            self._run_background_query_export, job_id, query, format, parameters
        )
    ...
    async def _run_background_table_export(
        ...
        try:
            ...
            export_dir = Path(
                settings.EXPORT_DIR
                if hasattr(settings, "EXPORT_DIR")
                else "/tmp/exports"
            )
