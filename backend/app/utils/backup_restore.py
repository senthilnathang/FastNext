...
class BackupManager:
    ...
    def __init__(self, session: AsyncSession, engine: Engine):
        ...
        self.storage_backend = getattr(settings, 'BACKUP_STORAGE_BACKEND', StorageBackend.LOCAL)
    ...
    async def create_backup(
        ...
        try:
            ...
            if estimated_size > 100 * 1024 * 1024 or background_tasks:  # 100MB threshold
    ...
    async def restore_backup(
        ...
        try:
            ...
            try:
                ...
                # Check compatibility
                compatibility_issues = await self._check_restore_compatibility(backup_metadata)
                ...
                if file_size > 50 * 1024 * 1024 or background_tasks:  # 50MB threshold
                    ...
                    job_id = await self._start_background_restore(
                        temp_path, backup_metadata, restore_strategy, tables, background_tasks
                    )
    ...
    async def _get_schema_version(self) -> str:
        ...
        try:
            ...
            result = await self.session.execute(text("SELECT version_num FROM alembic_version LIMIT 1"))
    ...
    async def _stream_backup(self, backup_metadata: Dict[str, Any]) -> StreamingResponse:
    ...
    async def _generate_backup_chunks(self, backup_metadata: Dict[str, Any]) -> AsyncGenerator[bytes, None]:
    ...
    async def _generate_sql_dump_chunks(self, backup_metadata: Dict[str, Any]) -> AsyncGenerator[bytes, None]:
    ...
    async def _generate_json_backup_chunks(self, backup_metadata: Dict[str, Any]) -> AsyncGenerator[bytes, None]:
    ...
    async def _generate_compressed_json_chunks(self, backup_metadata: Dict[str, Any]) -> AsyncGenerator[bytes, None]:
    ...
    async def _check_restore_compatibility(self, backup_metadata: Dict[str, Any]) -> List[str]:
        ...
        if backup_schema_version and backup_schema_version != current_schema_version:
            issues.append(f"Schema version mismatch: backup={backup_schema_version}, current={current_schema_version}")
        
        # Check database version compatibility
        backup_db_version = backup_metadata.get("database_version", "")
        current_db_version = await self._get_database_version()
        
        if backup_db_version and not self._is_db_version_compatible(backup_db_version, current_db_version):
            issues.append(f"Database version compatibility issue: backup={backup_db_version}, current={current_db_version}")
        
        return issues
    
    def _is_db_version_compatible(self, backup_version: str, current_version: str) -> bool:
    ...
    async def _start_background_backup(
        ...
        background_tasks.add_task(
            self._run_background_backup,
            job_id, backup_metadata
        )
    ...
    async def _start_background_restore(
        ...
        background_tasks.add_task(
            ...
            job_id, backup_path, backup_metadata, restore_strategy, tables
        )
        
        return job_id
    
    async def _run_background_backup(self, job_id: str, backup_metadata: Dict[str, Any]):
        ...
        try:
            ...
            if self.storage_backend != StorageBackend.LOCAL:
                await self._upload_backup_to_storage(backup_path, backup_metadata['backup_id'])
    ...
    async def _run_background_restore(
        ...
        try:
            ...
            # Execute restore
            result = await self._execute_restore(backup_path, backup_metadata, restore_strategy, tables)
    ...
    async def _execute_restore(
        ...
        if backup_format == BackupFormat.SQL_DUMP:
            return await self._restore_from_sql_dump(backup_path, restore_strategy, tables)
        elif backup_format in [BackupFormat.JSON, BackupFormat.COMPRESSED_JSON]:
            return await self._restore_from_json(backup_path, backup_metadata, restore_strategy, tables)
        else:
    ...
    async def _list_local_backups(
        ...
        limit: int,
        offset: int,
        format_filter: Optional[BackupFormat]
    ) -> List[Dict[str, Any]]:
