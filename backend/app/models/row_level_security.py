...
class RowLevelSecurityPolicy(Base):
    ...
    )  # Column to check (e.g., 'user_id', 'organization_id')
    condition_value_source = Column(
        String(100), nullable=True
    )  # Source of value (e.g., 'current_user.id', 'session.tenant_id')
...
class RLSRuleAssignment(Base):
    ...
    # Policy reference
    policy_id = Column(
        Integer, ForeignKey("rls_policies.id"), nullable=False, index=True
    )
    ...
    )  # Specific entity ID (null = applies to all)
...
class RLSAuditLog(Base):
    ...
    # RLS details
    policy_id = Column(
        Integer, ForeignKey("rls_policies.id"), nullable=True, index=True
    )
    entity_type = Column(Enum(RLSEntityType), nullable=False, index=True)
