...
class SystemConfiguration(AuditableModel):
    ...
    is_system_config = Column(Boolean, default=False, nullable=False)  # System vs user configurable
    requires_restart = Column(Boolean, default=False, nullable=False)  # If changes require app restart
    
    # Validation and defaults
    default_value = Column(JSON, default={}, nullable=False)
    validation_schema = Column(JSON, default={}, nullable=False)  # JSON schema for validation
    
    # Version control
    version = Column(String(20), default="1.0.0", nullable=False)
    previous_version = Column(JSON, default={}, nullable=False)  # Store previous config for rollback
...
class ConfigurationTemplate(AuditableModel):
    ...
    variables = Column(JSON, default=[], nullable=False)  # Template variables to be replaced
    ...
    # Environment support
    supported_environments = Column(JSON, default=["production", "staging", "development"], nullable=False)
...
class ConfigurationAuditLog(AuditableModel):
    ...
    action = Column(String(50), nullable=False, index=True)  # created, updated, deleted, applied, validated
