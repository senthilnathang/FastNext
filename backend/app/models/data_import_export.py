...
from .base import BaseModel, FullAuditModel
...
class ExportPermission(BaseModel):
    ...
    allowed_columns = Column(
        JSON, default=[], nullable=False
    )  # Empty means all columns allowed
...
class ImportAuditLog(BaseModel):
    ...
    )  # created, started, completed, failed, etc.
...
class ExportAuditLog(BaseModel):
    ...
    export_job_id = Column(
        Integer, ForeignKey("export_jobs.id"), nullable=False, index=True
    )
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    ...
    event_type = Column(
        String(50), nullable=False, index=True
    )  # created, started, completed, downloaded, etc.
    event_data = Column(JSON, default={}, nullable=False)
    ip_address = Column(String(45), nullable=True)
    user_agent = Column(String(500), nullable=True)
