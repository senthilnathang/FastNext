...
class ActivityLog(Base):
    ...
    entity_id = Column(
        String(100), nullable=True, index=True
    )  # Support for UUID and Integer IDs
    entity_name = Column(
    ...
    level = Column(
        Enum(ActivityLevel), default=ActivityLevel.INFO, nullable=False, index=True
    )
    ...
    )  # When event was processed/analyzed
    ...
    def to_event_dict(self):
        ...
        return {
            ...
            "location": (
                {"country": self.country_code, "city": self.city}
                if self.country_code or self.city
                else None
            ),
            "description": self.description,
            "metadata": self.event_metadata,
    ...
    def to_user_event_dict(self):
        ...
        return {
            ...
            "entity_id": (
                int(self.entity_id)
                if self.entity_id and str(self.entity_id).isdigit()
                else None
            ),
            "entity_name": self.entity_name,
            "description": self.description,
