...
class GraphQLMockRouter:
    ...
    def setup_routes(self):
        ...
        async def graphql_endpoint(
            ...
            try:
                ...
                return JSONResponse(
                    content={"errors": [{"message": str(e)}], "data": None},
                    status_code=400,
                )
    ...
    async def execute_query(
        ...
        if query.startswith("mutation"):
            ...
            return await self.execute_query_operation(
                query, variables, user, db, request
            )
    ...
    async def execute_query_operation(
        ...
        for operation in operations:
            try:
                ...
                errors.append({"message": str(e), "path": [operation["name"]]})
        ...
        if not operations:
            ...
            for match in simple_matches:
                if match in self.operation_map:
                    operations.append({"name": match, "arguments": {}, "fields": []})
    ...
    async def simulate_rest_call(
        ...
        try:
            ...
            if operation_name == "me":
                ...
                user_id = variables.get("id") or operation.get("arguments", {}).get(
                    "id"
                )
                if not user_id:
                ...
                project_id = variables.get("id") or operation.get("arguments", {}).get(
                    "id"
                )
                if not project_id:
                ...
                return await service.get_assets(project_id=variables.get("projectId"))
            ...
            raise HTTPException(
                status_code=500, detail=f"Error executing {operation_name}: {str(e)}"
            )
