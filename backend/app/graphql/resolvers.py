...
def convert_component_to_graphql(component: Component) -> ComponentType:
    ...
    return ComponentType(
        ...
        project=(
            convert_project_to_graphql(component.project) if component.project else None
        ),
    )
...
class Query:
    ...
    async def project_members(
        self, info: strawberry.Info[GraphQLContext], project_id: int
    ...
    @strawberry.field
    async def assets(
        self, info: strawberry.Info[GraphQLContext], project_id: Optional[int] = None
    ) -> List[AssetType]:
        """Get assets"""
        db = info.context.db
