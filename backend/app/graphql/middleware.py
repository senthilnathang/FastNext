...
class GraphQLMiddleware(BaseHTTPMiddleware):
    ...
    async def handle_graphql_query(self, request: Request, call_next: Callable) -> Response:
    ...
    async def handle_graphql_header(self, request: Request, call_next: Callable) -> Response:
            ...
            for key, value in data.items():
                if key in selected_fields:
                    if isinstance(selected_fields[key], dict):
                        # Recursive selection
                        filtered[key] = self.apply_field_selection(value, selected_fields[key])
                    else:
                        # Include field
                        filtered[key] = value
            return filtered
    ...
    async def apply_graphql_response(self, response: Response, selected_fields: Dict[str, Any]) -> Response:
...
class GraphQLQueryEnhancer:
    ...
    @staticmethod
    def add_graphql_info_to_response(response_data: Dict[str, Any], endpoint: str) -> Dict[str, Any]:
        ...
        graphql_info = {
            "graphql": {
                ...
                "equivalent_query": GraphQLQueryEnhancer.get_equivalent_graphql_query(endpoint),
