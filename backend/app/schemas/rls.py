...
from app.models.row_level_security import RLSAction, RLSEntityType, RLSPolicy
...
class RLSPolicyBase(BaseModel):
    ...
    condition_column: Optional[str] = Field(
        None, max_length=100, description="Condition column"
    )
    condition_value_source: Optional[str] = Field(
        None, max_length=100, description="Condition value source"
    )
...
class RLSRuleAssignmentCreate(RLSRuleAssignmentBase):
    ...
    def validate_assignment_target(cls, v, values):
        ...
        if "user_id" in values and "role_id" in values and "entity_id" in values:
            if not any(
                [values.get("user_id"), values.get("role_id"), values.get("entity_id")]
            ):
                raise ValueError(
                    "At least one of user_id, role_id, or entity_id must be specified"
                )
        return v
...
class RLSAccessCheckResponse(BaseModel):
    ...
    denial_reason: Optional[str] = Field(
        None, description="Reason for denial if access denied"
    )
...
class RLSPolicyApplication(BaseModel):
    ...
    entity_ids: List[int] = Field(
        ..., description="List of entity IDs to apply policy to"
    )
    conditions: Optional[Dict[str, Any]] = Field(
        None, description="Additional conditions"
    )
...
class RLSBulkPolicyCreate(BaseModel):
    ...
    policies: List[RLSPolicyCreate] = Field(
        ..., description="List of policies to create"
    )
    apply_immediately: bool = Field(
        True, description="Whether to apply policies immediately"
    )
...
class RLSSecurityAnalysis(BaseModel):
    ...
    coverage_percentage: float = Field(
        ..., description="Percentage of entities covered by RLS"
    )
...
class RLSConfiguration(BaseModel):
    ...
    default_policy: RLSPolicy = Field(
        RLSPolicy.OWNER_ONLY, description="Default policy for new entities"
    )
    ...
    strict_mode: bool = Field(
        False, description="Whether to use strict mode (deny by default)"
    )
...
class RLSExportRequest(BaseModel):
    ...
    include_assignments: bool = Field(
        True, description="Include rule assignments in export"
    )
    ...
    entity_types: Optional[List[RLSEntityType]] = Field(
...
class RLSImportRequest(BaseModel):
    ...
    overwrite_existing: bool = Field(
        False, description="Whether to overwrite existing policies"
    )
