...
class AuditTrailBase(BaseModel):
    entity_type: str = Field(
        ..., max_length=100, description="Type of entity being audited"
    )
    entity_id: int = Field(..., description="ID of the audited entity")
    entity_name: Optional[str] = Field(
        None, max_length=255, description="Name of the audited entity"
    )
    operation: str = Field(
        ..., max_length=50, description="Operation performed (INSERT, UPDATE, DELETE)"
    )
...
class AuditTrailCreate(AuditTrailBase):
    ...
    old_values: Optional[str] = Field(
        None, description="Previous values as JSON string"
    )
    new_values: Optional[str] = Field(None, description="New values as JSON string")
    changed_fields: Optional[str] = Field(
        None, description="Changed fields as JSON array"
    )
...
class AuditTrailFilter(BaseModel):
    ...
    changed_fields_contain: Optional[str] = Field(
        None, description="Filter by field name"
    )
