...
ROUTE_PERMISSIONS: Dict[str, Dict[str, Any]] = {
    ...
    "/api/v1/admin": {"requires_admin": True, "permissions": ["admin.access"]},
    "/api/v1/users": {"requires_admin": True, "permissions": ["user.manage"]},
    "/api/v1/roles": {"requires_admin": True, "permissions": ["role.manage"]},
    "/api/v1/permissions": {
        "requires_admin": True,
        "permissions": ["permission.manage"],
    },
    # Data management routes
    "/api/v1/data/import": {"permissions": ["data.import"]},
    "/api/v1/data/export": {"permissions": ["data.export"]},
    # Configuration routes
    "/api/v1/config": {"permissions": ["config.manage"]},
    # Project routes - basic access
    "/api/v1/projects": {"permissions": ["project.read"]},
    # Workflow routes
    "/api/v1/workflows": {"permissions": ["workflow.read"]},
    # Profile routes - self-access
    "/api/v1/profile": {"permissions": ["profile.edit"]},
    # System monitoring
    "/api/v1/system": {"requires_admin": True, "permissions": ["system.monitor"]},
    # Audit and security
    "/api/v1/audit": {"requires_admin": True, "permissions": ["audit.read"]},
    "/api/v1/security": {"requires_admin": True, "permissions": ["security.manage"]},
}
    ...
    if route_perms.get("requires_admin", False):
        if not current_user.is_superuser:
            if not PermissionService.check_permission(
                db, current_user.id, "manage", "system"
            ):
                logger.warning(
                    f"❌ Admin access denied for user {current_user.id} to {path}"
                )
                raise HTTPException(
...
AUTH_SUMMARY = {
    ...
    "admin_endpoints": [
        route
        for route, perms in ROUTE_PERMISSIONS.items()
        if perms.get("requires_admin")
    ],
