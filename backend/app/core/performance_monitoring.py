...
class MetricsCollector:
    ...
    def __init__(self, max_data_points: int = 10000):
        ...
        self.metrics: Dict[MetricType, deque] = {
            metric_type: deque(maxlen=max_data_points)
    ...
    def add_metric(self, metric_type: MetricType, value: float, labels: Optional[Dict[str, str]] = None):
    ...
    async def _monitor_loop(self, interval: float):
        ...
        while self.monitoring:
            try:
                ...
                self.collector.add_metric(MetricType.MEMORY_USAGE, disk_usage, {"type": "disk"})
...
class AlertManager:
    ...
    async def start_monitoring(self, check_interval: float = 30.0):
        ...
        self.monitoring = True
        self._monitor_task = asyncio.create_task(self._alert_monitor_loop(check_interval))
...
class PerformanceReporter:
    ...
    def generate_summary_report(self, time_window: timedelta = timedelta(hours=1)) -> Dict[str, Any]:
        ...
        slowest_endpoints = sorted(
            [(endpoint, stats["max_time"]) for endpoint, stats in endpoint_stats.items()],
            key=lambda x: x[1],
            reverse=True
        )[:10]
        ...
        error_endpoints = sorted(
            [(endpoint, stats["error_rate"]) for endpoint, stats in endpoint_stats.items()],
            key=lambda x: x[1],
            reverse=True
        )[:10]
