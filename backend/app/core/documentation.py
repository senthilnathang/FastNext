        ...
        if "content" in request_body:
            for media_type, content in request_body["content"].items():
                if media_type == "application/json":
                    # Add generic example
                    content["example"] = {"example_field": "example_value"}
    ...
    def _setup_custom_docs_routes(self):
        ...
        async def get_postman_collection():
            ...
            return JSONResponse(
                content=collection,
                headers={
                    "Content-Disposition": "attachment; filename=fastnext-collection.json"
                },
            )
        ...
        async def get_insomnia_collection():
            ...
            return JSONResponse(
                content=collection,
                headers={
                    "Content-Disposition": "attachment; filename=fastnext-insomnia.json"
                },
            )
                ...
                if isinstance(operation, dict):
                    ...
                    if "requestBody" in operation:
                        ...
                        if (
                            ...
                            example = request_body["content"]["application/json"].get(
                                "example", {}
                            )
...
class APIChangelogGenerator:
    ...
    def generate_changelog(
        self, current_schema: Dict[str, Any], version: str
    ) -> Dict[str, Any]:
        """Generate changelog between schema versions"""

        if version not in self.previous_schemas:
            return {"version": version, "changes": [], "summary": "Initial API version"}
    ...
    def _compare_schemas(
        ...
        for path in old_paths & new_paths:
            path_changes = self._compare_path(
                old_schema["paths"][path], new_schema["paths"][path], path
            )
    ...
    def _compare_path(
        self, old_path: Dict[str, Any], new_path: Dict[str, Any], path: str
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_summary(self, changes: List[Dict[str, Any]]) -> str:
        ...
        if modified > 0:
            summary_parts.append(
                f"{modified} modification{'s' if modified != 1 else ''}"
            )
        ...
        if breaking > 0:
            summary_parts.append(
                f"{breaking} breaking change{'s' if breaking != 1 else ''}"
            )
    ...
    async def export_markdown(self) -> str:
        ...
        if "servers" in schema:
            ...
            for server in schema["servers"]:
                markdown.append(
                    f"- `{server['url']}` - {server.get('description', '')}"
                )
            markdown.append("")
            ...
            for method, operation in methods.items():
                ...
                if "parameters" in operation:
                    ...
                    for param in operation["parameters"]:
                        ...
                        markdown.append(
                            f"- `{param['name']}` ({param['in']}){required}: {param.get('description', '')}"
                        )
...
<body>
        ...
        for path, methods in schema.get("paths", {}).items():
            ...
            for method, operation in methods.items():
                ...
                html.append(
                    f'<h4><span class="method {method}">{method.upper()}</span> {path}</h4>'
                )

                if "summary" in operation:
                    html.append(
                        f"<p><strong>Summary:</strong> {operation['summary']}</p>"
                    )
