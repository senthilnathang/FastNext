        ...
        try:
            async with self.manager.get_redis() as redis_client:
                ...
                if json_serialized:
                    ...
                    value = pickle.loads(serialized_value.encode() if isinstance(serialized_value, str) else serialized_value)
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if key_builder:
                ...
                cache_key = f"{key_prefix}{func_name}_{args_str}_{kwargs_str}".replace(" ", "_")
        ...
        try:
            async with self.manager.get_redis() as redis_client:
                ...
                if current_count >= limit:
                    ...
                    oldest_entries = await redis_client.zrange(f"rate_limit:{key}", 0, 0, withscores=True)
                    reset_time = oldest_entries[0][1] + window if oldest_entries else now + window
