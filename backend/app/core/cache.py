...
class MemoryCache(CacheInterface):
    ...
    async def _ensure_capacity(self, new_size: int):
        ...
        while (len(self._cache) >= self.config.max_size or 
               self._stats.total_size + new_size > self.config.max_size * 1024):  # Assume 1KB per entry max
...
class HybridCache(CacheInterface):
    ...
    def __init__(self, config: CacheConfig):
        self.config = config
        self.l1_cache = MemoryCache(CacheConfig(
            backend=CacheBackend.MEMORY,
            max_size=config.max_size // 2,  # L1 cache is smaller
            default_ttl=config.default_ttl
        ))
        self.l2_cache = RedisCache(config) if REDIS_AVAILABLE and config.redis_url else None
        self._stats = CacheStats()
    ...
    async def get_stats(self) -> CacheStats:
        ...
        combined_stats.deletes = l1_stats.deletes + l2_stats.deletes + self._stats.deletes
...
class CacheWarmer:
    ...
    async def warm_from_function(
        ...
        for i in range(0, len(keys), batch_size):
            ...
            for key in batch:
                try:
                    value = await value_func(key) if asyncio.iscoroutinefunction(value_func) else value_func(key)
...
def initialize_caches():
    ...
    try:
        ...
        default_config = CacheConfig(
            backend=CacheBackend.HYBRID if REDIS_AVAILABLE and settings.REDIS_URL else CacheBackend.MEMORY,
        ...
        cache_manager.create_cache("users", CacheConfig(
        ...
        cache_manager.create_cache("sessions", CacheConfig(
            backend=default_config.backend,
        ...
        cache_manager.create_cache("api_responses", CacheConfig(
            backend=default_config.backend,
