...
'''
        ...
        self._write_file(
            f"app/auth/permissions/{self.snake_name}_permissions.py", content
        )
...
""".format(
            snake_name=self.snake_name, model_name=self.model_name
        )

        self._write_file(
            f"app/auth/middleware/{self.snake_name}_middleware.py", content
        )
...
'''
    ...
    def get_basic_fields(cls) -> List[str]:
        ...
        for field in self.model_def.fields:
            if (
                field.name not in ["password", "secret", "token", "key"]
                and field.type != FieldType.EMAIL
            ):
                content += f'"{field.name}", '
...
"""
        ...
        self._write_file(
            f"app/auth/field_access/{self.snake_name}_field_access.py", content
        )
