...
'''
    ...
    def test_{self.snake_name}_update_schema(self, {self.snake_name}_data):
        ...
        sample_fields = [
            f.name
            for f in self.model_def.fields[:3]
...
'''
        ...
        for field in self.model_def.fields:
            if field.type in [FieldType.STRING, FieldType.TEXT] and field.name not in [
                "id"
            ]:
...
'''
        ...
        sensitive_fields = [
            f
            for f in self.model_def.fields
            if f.name in ["password", "secret", "token", "key"]
            or f.type == FieldType.EMAIL
        ]
...
"""
        ...
        self._write_file(
            f"tests/integration/test_{self.snake_name}_permissions.py", content
        )
...
'''
        ...
        self._write_file(
            f"tests/performance/test_{self.snake_name}_performance.py", content
        )
