...
class TestWorkflowTemplateAPI:
    ...
    def test_get_workflow_templates_by_type(self, test_data):
        ...
        response = client.get(
            f"/api/workflow-templates/?workflow_type_id={workflow_type_id}"
        )
        assert response.status_code == 200
        ...
        assert len(data["items"]) >= 1
        assert all(
            item["workflow_type_id"] == workflow_type_id for item in data["items"]
        )
    ...
    def test_update_workflow_template(self, test_data):
        ...
        response = client.put(
            f"/api/workflow-templates/{template_id}", json=update_data
        )
        assert response.status_code == 200
...
class TestWorkflowAPIErrors:
    ...
    def test_duplicate_workflow_type_name(self, test_data):
        ...
        workflow_type_data = {"name": "Duplicate Test", "description": "First instance"}
    ...
    def test_duplicate_workflow_state_name(self, test_data):
        ...
        state_data = {"name": "duplicate_state", "label": "Duplicate State"}
...
class TestWorkflowAPIPagination:
    ...
    def test_search_functionality(self, test_data):
        ...
        found_items = [
            ...
            or "invoice" in (item["description"] or "").lower()
        ]
