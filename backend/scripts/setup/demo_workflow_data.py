    ...
    try:
        ...
        for wt_data in workflow_types:
            existing = (
                ...
                .filter(WorkflowType.name == wt_data["name"])
                .first()
            )
            if not existing:
        ...
        workflow_states = [
            ...
            },
            {
                ...
                "description": "Setting up equipment and accounts",
                "color": "#0EA5E9",
                "bg_color": "#E0F2FE",
                "icon": "Monitor",
            },
            {
                "name": "training",
                "label": "Training",
                "description": "Employee training period",
                "color": "#F59E0B",
                "bg_color": "#FFFBEB",
                "icon": "GraduationCap",
            },
            ...
            },
            {
                "name": "triaged",
                "label": "Triaged",
                "description": "Bug has been triaged",
                "color": "#F59E0B",
                "bg_color": "#FFFBEB",
                "icon": "Filter",
            },
        ...
        for ws_data in workflow_states:
            existing = (
                ...
                .filter(WorkflowState.name == ws_data["name"])
                .first()
            )
            if not existing:
                ...
                "edges": [
                    {
                        ...
                        "target": "amount_check",
                        "type": "smoothstep",
                        "data": {"action": "submit", "label": "Submit"},
                    },
                    {
                        "id": "ie2",
                        "source": "amount_check",
                        ...
                        "type": "smoothstep",
                        "data": {"action": "high_amount", "label": "Amount ≥ $1000"},
                    },
                    {
                        "id": "ie3",
                        "source": "amount_check",
                        "target": "auto_approve",
                        "type": "smoothstep",
                        "data": {"action": "low_amount", "label": "Amount < $1000"},
                    },
                    {
                        "id": "ie4",
                        ...
                        "type": "smoothstep",
                        "data": {"action": "approve", "label": "Manager Approved"},
                    },
                    {
                        "id": "ie5",
                        "source": "finance_review",
                        "target": "approved_final",
                        "type": "smoothstep",
                        "data": {"action": "approve", "label": "Finance Approved"},
                    },
                    {
                        "id": "ie6",
                        "source": "auto_approve",
                        "target": "approved_final",
                        "type": "smoothstep",
                        "data": {"action": "auto_process", "label": "Auto Process"},
                    },
                ],
                "settings": {
                ...
                "edges": [
                    {
                        ...
                        "target": "triage_task",
                        "type": "smoothstep",
                        "data": {"action": "triage", "label": "Assign for Triage"},
                    },
                    {
                        "id": "be2",
                        "source": "triage_task",
                        "target": "priority_gateway",
                        "type": "smoothstep",
                        "data": {"action": "triaged", "label": "Triaged"},
                    },
                    {
                        "id": "be3",
                        "source": "priority_gateway",
                        "target": "high_priority_dev",
                        "type": "smoothstep",
                        "data": {"action": "high_priority", "label": "High Priority"},
                    },
                    {
                        "id": "be4",
                        "source": "priority_gateway",
                    ...
                    {
                        "id": "be7",
                        "source": "testing_phase",
                        "target": "bug_resolved",
                        "type": "smoothstep",
                        "data": {"action": "test_pass", "label": "Tests Passed"},
                    },
                ],
                "settings": {
                    "sla_enabled": True,
        ...
        for wt_data in workflow_templates:
            existing = (
                ...
                .filter(WorkflowTemplate.name == wt_data["name"])
                .first()
            )
            if not existing:
        ...
        for instance_data in sample_instances:
            existing = (
                ...
                .filter(WorkflowInstance.entity_id == instance_data["entity_id"])
                .first()
            )
            if not existing:
        ...
        print(
            f"📊 Created: {len(created_types)} workflow types, {len(created_states)} states, {len(created_templates)} templates"
        )
