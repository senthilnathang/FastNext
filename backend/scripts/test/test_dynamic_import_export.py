            ...
            if response.status_code == 200:
                ...
                if columns:
                    ...
                    for col in columns:
                        ...
                        print(
                            f"     * {col.get('name')}: {col.get('type')} ({nullable}){pk}"
                        )
            else:
    ...
    if tables and "users" in tables:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   - Import Max File Size: {import_perm.get('max_file_size_mb', 0)} MB"
                )
                print(
                    f"   - Export Max Rows: {export_perm.get('max_rows_per_export', 0)}"
                )
