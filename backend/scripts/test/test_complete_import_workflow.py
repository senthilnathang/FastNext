...
def test_file_upload_endpoint():
    ...
    try:
        ...
        # Test CSV parsing
        parsed_csv = _parse_uploaded_file(csv_content.encode('utf-8'), csv_filename, options)
        
        print(f"✅ CSV parsing successful:")
        print(f"   Headers: {parsed_csv.get('headers', [])}")
        print(f"   Rows: {len(parsed_csv.get('rows', []))}")
        print(f"   First row: {parsed_csv.get('rows', [{}])[0] if parsed_csv.get('rows') else 'None'}")
        ...
        parsed_json = _parse_uploaded_file(json_content.encode('utf-8'), json_filename, json_options)
        
        print(f"✅ JSON parsing successful:")
        print(f"   Headers: {parsed_json.get('headers', [])}")
        print(f"   Rows: {len(parsed_json.get('rows', []))}")
        print(f"   First row: {parsed_json.get('rows', [{}])[0] if parsed_json.get('rows') else 'None'}")
...
def test_validation_system():
    ...
    try:
        from app.api.v1.data_import_export import _validate_import_data, _get_model_class
        ...
        try:
            ...
            if not model_class:
                print(f"⚠️  No model found for '{table_name}', using fallback validation")
                return True
            ...
            test_data = {
                ...
                "rows": [
                    {"name": "Test Project", "description": "Test Description", "status": "active", "budget": "10000.00"},
                    {"name": "", "description": "Missing name", "status": "active", "budget": "5000.00"},  # Invalid: missing name
                    {"name": "Valid Project", "description": "Valid description", "status": "invalid_status", "budget": "abc"}  # Invalid: bad status and budget
                ]
            }
...
def test_database_import():
    ...
    try:
        from app.api.v1.data_import_export import _perform_data_import, _get_model_class
        from app.core.config import settings
        from sqlalchemy import create_engine
        from sqlalchemy.orm import sessionmaker
        ...
        try:
            ...
            import_job.import_results = {
                "parsed_data": {
                    ...
                    "rows": [
                        {"name": "Test Import Project", "description": "Testing import", "status": "active"}
                    ]
                },
                "validation_results": {
                    "valid_rows": 1,
...
def test_error_handling():
    ...
    try:
        ...
        try:
            result = _parse_uploaded_file(invalid_csv.encode('utf-8'), "test.csv", options)
        ...
        try:
            result = _parse_uploaded_file(invalid_json.encode('utf-8'), "test.json", json_options)
...
if __name__ == "__main__":
    ...
    if success1 and success2 and success3 and success4 and success5 and success6:
        ...
        failed_tests = []
        if not success1: failed_tests.append("File Upload")
        if not success2: failed_tests.append("Validation System")
        if not success3: failed_tests.append("Database Import")
        if not success4: failed_tests.append("Temp File Handling")
        if not success5: failed_tests.append("Error Handling")
        if not success6: failed_tests.append("Workflow Integration")
