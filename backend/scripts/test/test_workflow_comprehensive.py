...
class WorkflowTester:
    ...
    def test_workflow_types_crud(self):
        ...
        ), f"GET workflow types failed: {response.text}"
        ...
        response = self.client.post(
            "/api/v1/workflow-types", json=new_type_data, headers=self.auth_headers
        )
        assert (
            response.status_code == 200
        ), f"POST workflow type failed: {response.text}"
        ...
        response = self.client.get(
            f"/api/v1/workflow-types/{type_id}", headers=self.auth_headers
        )
    ...
    def test_workflow_templates_crud(self):
        ...
        types_response = self.client.get(
            "/api/v1/workflow-types", headers=self.auth_headers
        )
        types_data = types_response.json()
        workflow_type_id = types_data["items"][0]["id"]

        states_response = self.client.get(
            "/api/v1/workflow-states", headers=self.auth_headers
        )
        states_data = states_response.json()
        initial_state_id = next(
            s["id"] for s in states_data["items"] if s["is_initial"]
        )

        # Test GET /api/v1/workflow-templates
        response = self.client.get(
            "/api/v1/workflow-templates", headers=self.auth_headers
        )
        assert response.status_code == 200
        ...
        new_template_data = {
            ...
            "settings": {"auto_advance": False, "notifications": True},
        }
        ...
        print(
            f"✅ Created workflow template: {created_template['name']} (ID: {template_id})"
        )

        # Test GET single template
        response = self.client.get(
            f"/api/v1/workflow-templates/{template_id}", headers=self.auth_headers
        )
        assert response.status_code == 200
    ...
    def test_workflow_instances_crud(self):
        ...
        response = self.client.get(
            "/api/v1/workflow-instances", headers=self.auth_headers
        )
        assert response.status_code == 200
        ...
        new_instance_data = {
            ...
            "data": {"test_field": "test_value", "priority": "high"},
            "context": {"created_by_test": True},
            "priority": 1,
        }
        ...
        print(
            f"✅ Created workflow instance: {created_instance['title']} (ID: {instance_id})"
        )

        # Test GET single instance
        response = self.client.get(
            f"/api/v1/workflow-instances/{instance_id}", headers=self.auth_headers
        )
        assert response.status_code == 200
    ...
    def test_workflow_advanced_features(self):
        ...
        response = self.client.get(
            "/api/v1/workflow-analytics", headers=self.auth_headers
        )
