...
def test_audit_logging():
    ...
    try:
        ...
        result = db.execute(
            text(
                "SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'import_audit_logs'"
            )
        )
        ...
        try:
            _log_import_audit(
                db, mock_job_id, mock_user_id, "file_uploaded", test_event_data
            )
        ...
        latest_log = (
            ...
            .filter(ImportAuditLog.event_type == "file_uploaded")
        ...
        event_types_to_test = [
            ...
            (
                "validation_started",
                {"total_rows": 100, "validation_stage": "data_validation_initiated"},
            ),
            (
                "validation_completed",
                {"is_valid": True, "valid_rows": 95, "error_rows": 5},
            ),
            ("import_started", {"total_rows": 100, "table_name": "test_table"}),
            (
                "import_completed",
                {"processed_rows": 95, "valid_rows": 95, "error_rows": 0},
            ),
            (
                "import_failed",
                {"error_message": "Test error", "failure_stage": "import_execution"},
            ),
        ]
        ...
        total_logs = (
            db.query(ImportAuditLog)
            .filter(ImportAuditLog.import_job_id == mock_job_id)
            .count()
        )
