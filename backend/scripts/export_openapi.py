...
def create_postman_collection(openapi_schema: Dict[str, Any], output_file: str = "fastnext-postman-collection.json"):
    ...
    for path, methods in paths.items():
        for method, details in methods.items():
            if method.lower() in ['get', 'post', 'put', 'delete', 'patch']:
                ...
                if 'security' in details or any(resp_code.startswith('401') for resp_code in details.get('responses', {})):
                ...
                if method.lower() in ['post', 'put', 'patch']:
                    ...
                    if request_body:
                        ...
                        if 'application/json' in content:
                            ...
                            example = create_example_from_schema(schema, openapi_schema.get('components', {}))
                            if example:
...
def create_example_from_schema(schema: Dict[str, Any], components: Dict[str, Any]) -> Any:
...
def create_insomnia_collection(openapi_schema: Dict[str, Any], output_file: str = "fastnext-insomnia-collection.json"):
...
def main():
    ...
    try:
        ...
        create_postman_collection(openapi_schema, str(exports_dir / "fastnext-postman-collection.json"))
        
        # Create Insomnia collection
        create_insomnia_collection(openapi_schema, str(exports_dir / "fastnext-insomnia-collection.json"))
